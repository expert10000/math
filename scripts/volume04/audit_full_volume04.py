...
def main():
    ...
    total_p=total_e=total_h=total_s=0
    for n in range(1,32):
        code=f"IV/{n:02d}"
        row=next((r for r in status if r.get("chapter_code")==code),None)
        if not row:
            blockers.append(f"MISSING_STATUS:{code}")
            continue
        ...
        live_count=sum(1 for r in src if r.get("destination")==code)
        try:
            ledger_count=int(row.get("mapped_rule_count") or 0)
        except Exception:
            ledger_count=-1
        if ledger_count!=live_count:
            blockers.append(f"MAPPED_RULE_COUNT:{code}:{ledger_count}!={live_count}")
    ...
    missingrefs=sorted({x for x in refs if x not in labelset})
    if dups:
        blockers.append("DUPLICATE_LABELS:"+",".join(dups[:30]))
    if missingrefs:
        blockers.append("MISSING_REFS:"+",".join(missingrefs[:30]))

    batches=["IV01_IV06","IV07_IV11","IV12_IV18","IV19_IV26","IV27_IV31"]
    accounting=[];provenance=[]
    for tag in batches:
        apath=rec/f"VOLUME04_{tag}_SOURCE_RULE_ACCOUNTING.tsv"
        ppath=rec/f"VOLUME04_{tag}_DOSSIER_PROVENANCE.tsv"
        if not apath.exists():
            blockers.append(f"MISSING_ACCOUNTING:{tag}")
        else:
            accounting+=tsv(apath)
        if not ppath.exists():
            blockers.append(f"MISSING_PROVENANCE:{tag}")
        else:
            provenance+=tsv(ppath)

    live_rules=[r for r in src if re.fullmatch(r"IV/\d{2}",r.get("destination",""))]
    if len(accounting)!=len(live_rules):
        blockers.append(f"ACCOUNTING_COUNT:{len(accounting)}!={len(live_rules)}")
    if len(provenance)!=372:
        blockers.append(f"PROVENANCE_COUNT:{len(provenance)}!=372")

    unresolved=[r for r in accounting if r.get("disposition","").startswith("UNRESOLVED")]
    if unresolved:
        blockers.append(f"UNRESOLVED_SOURCE_RULES:{len(unresolved)}")
    missing_sources=[r for r in live_rules if not source_exists(repo,r.get("source_file",""))]
    if missing_sources:
        blockers.append(f"MISSING_SOURCE_FILES:{len(missing_sources)}")
    ...
    if (repo/"scripts/volume04/__pycache__").exists():
    ...
    (rec/"VOLUME04_FULL_RECONCILIATION.md").write_text("\n".join(md)+"\n",encoding="utf-8")
