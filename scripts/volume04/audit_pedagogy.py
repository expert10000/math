...
MEASURABLE={'analyze','apply','audit','build','check','choose','classify','combine','compactify','compare','compute','construct','continue','count','define','deform','derive','determine','differentiate','distinguish','estimate','explain','express','extend','factor','handle','identify','lift','locate','manipulate','map','parameterize','produce','prove','reconstruct','relate','select','show','specialize','state','test','track','translate','use','verify','write'}
BANNED_GOALS=(
"the reader should be able to state the definitions precisely, prove the core results",
"choose valid contours or local expansions",
"explain and use",
)
BANNED_HINTS=(
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--repo",required=True);ap.add_argument("--require-goals",action="store_true");ap.add_argument("--require-hints-through",type=int,default=0);ap.add_argument("--require-full",action="store_true");a=ap.parse_args()
    repo=Path(a.repo).resolve(); rs=[r for r in rows(repo) if r.get("volume")=="IV"]
    blockers=[]; report=[]; allh=[]; totals={"goals":0,"problems":0,"exercises":0,"hints":0,"solutions":0}; generic=0
    if len(rs)!=31:blockers.append(f"STATUS_ROWS:{len(rs)}!=31")
    for r in rs:
        ...
        probs=re.findall(r"\\begin\{problem\}",text); exs=re.findall(r"\\begin\{exercise\}",text); hs=[re.sub(r"\s+"," ",x).strip() for x in re.findall(r"\\begin\{hint\}(.*?)\\end\{hint\}",text,re.S)]; sols=re.findall(r"\\begin\{solution\}",text)
        ...
        if n<=a.require_hints_through or a.require_full:
            ...
            for i,h in enumerate(hs,1):
                ...
                if len(h)>760:blockers.append(f"{c}:HINT_{i:02d}_TOO_LONG:{len(h)}")
        if a.require_full:
    ...
    if a.require_full:
        exp={"goals":186,"problems":372,"exercises":248,"hints":248,"solutions":620}
    ...
    with (out/"VOLUME04_PEDAGOGY_AUDIT.tsv").open("w",encoding="utf-8",newline="") as f:
        w=csv.DictWriter(f,fieldnames=fields,delimiter="\t",lineterminator="\n");w.writeheader();w.writerows(report)
    s={"status":"PASS" if not blockers else "FAIL","chapters":len(rs),"learning_outcomes":totals["goals"],"problems":totals["problems"],"exercises":totals["exercises"],"hints":totals["hints"],"solutions":totals["solutions"],"generic_hint_hits":generic,"duplicate_hint_texts":len(dup),"riemann_surface_arc":"IV/22-IV/31","required_hints_through":31 if a.require_full else a.require_hints_through,"blocking":blockers}
    (out/"VOLUME04_PEDAGOGY_AUDIT.json").write_text(json.dumps(s,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    md=["# Volume IV Pedagogy Audit","",f"**Result:** {s['status']}","",f"- Chapters: **{len(rs)} / 31**",f"- Measurable learning outcomes: **{totals['goals']}**",f"- Solved dossiers/problems: **{totals['problems']}**",f"- Exercises / hints / solutions: **{totals['exercises']} / {totals['hints']} / {totals['solutions']}**",f"- Generic hint hits: **{generic}**",f"- Duplicate hint texts: **{len(dup)}**","","## Special arc","","IV/22–IV/31: analytic continuation to Riemann surfaces, coverings, gluing, genus, complex tori, elliptic functions, Weierstrass theory, and elliptic curves.","","## Blocking findings",""]
    md += [f"- {b}" for b in blockers] if blockers else ["None."]
    (out/"VOLUME04_PEDAGOGY_AUDIT.md").write_text("\n".join(md)+"\n",encoding="utf-8")
