...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--repo",required=True); ap.add_argument("--pdf",required=True); ap.add_argument("--log",required=True)
    a=ap.parse_args(); repo=Path(a.repo).resolve(); pdf=Path(a.pdf); log=Path(a.log); out=repo/REPORT; out.mkdir(parents=True,exist_ok=True)
    rows=[]; hashes=[]; labels=[]
    for i,p in enumerate(sorted((repo/VOL).glob("ch*/chapter.tex")),1):
        t=p.read_text(encoding="utf-8-sig")
        row={"chapter":f"III/{i:02d}","examples":cnt(t,"example"),"exercises":cnt(t,"exercise"),
             "hints":cnt(t,"hint"),"problems":cnt(t,"problem"),"solutions":cnt(t,"solution"),"labels":len(LABEL.findall(t))}
    ...
    (out/"VOLUME03_EXAMPLE_EXERCISE_RECONCILIATION.json").write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
    with (out/"VOLUME03_EXAMPLE_EXERCISE_COUNTS.tsv").open("w",encoding="utf-8",newline="") as f:
    ...
    with (out/"VOLUME03_EXAMPLE_EXERCISE_HASHES.tsv").open("w",encoding="utf-8",newline="") as f:
        w=csv.DictWriter(f,fieldnames=["chapter","path","sha256"],delimiter="\t"); w.writeheader(); w.writerows(hashes)
    md=["# Volume III pedagogy reconciliation after example/exercise expansion","",f"**Result:** {status}","","## Totals",""]
    for k,v in totals.items(): md.append(f"- {k}: **{v}**")
    md+=["","## Canonical PDF","",f"- path: `{pdfinfo['path']}`",f"- bytes: `{pdfinfo['bytes']}`",f"- SHA-256: `{pdfinfo['sha256']}`","","## Blocking findings",""]
    md += [f"- {b}" for b in blockers] if blockers else ["None."]
    (out/"VOLUME03_EXAMPLE_EXERCISE_RECONCILIATION.md").write_text("\n".join(md)+"\n",encoding="utf-8")
