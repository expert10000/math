...
EXAMPLE_BLOCK=re.compile(r"% BEGIN VOL03-EXPANSION (III\d\d-example-\d\d)\n(.*?)% END VOL03-EXPANSION \1",re.S)
...
def main():
    ...
    baseline=repo/REPORT/"VOLUME03_EXAMPLE_EXERCISE_BASELINE.json"
    ...
    for i,p in enumerate(fs,1):
        ...
        exblocks=list(EXAMPLE_BLOCK.finditer(t)); placed=[]
        for b in exblocks:
        ...
        if c["example"]<7: blockers.append(f"{code}:EXAMPLES:{c['example']}<7")
        ...
        if old and old.get("protected_sha256")!=protected: blockers.append(f"{code}:PROTECTED_TEXT_CHANGED")
        if old and c["example"]<old.get("examples",0): blockers.append(f"{code}:EXAMPLE_COUNT_DECREASE")
        if old and c["exercise"]<old.get("exercises",0): blockers.append(f"{code}:EXERCISE_COUNT_DECREASE")
        rows.append({"chapter":code,**c,"expansion_examples":len(exblocks),
                     "placed_after":" | ".join(placed),"protected_sha256":protected})
    for lab,n in Counter(x for x,_ in all_labels).items():
    ...
    if a.write:
        ...
        (out/"VOLUME03_EXAMPLE_EXERCISE_BALANCE_AUDIT.json").write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
        md=["# Volume III worked-example placement and graded-exercise balance","",
            f"**Result:** {status}","",
        ...
        (out/"VOLUME03_EXAMPLE_EXERCISE_BALANCE_AUDIT.md").write_text("\n".join(md)+"\n",encoding="utf-8")
