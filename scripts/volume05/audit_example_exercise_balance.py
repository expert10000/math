#!/usr/bin/env python3
import argparse,hashlib,json,re
from collections import Counter
from pathlib import Path
VOL=Path("books/vol05_commutative_algebra/chapters"); REPORT=Path("reports/series")
BLOCK=re.compile(r"% BEGIN VOL05-EXPANSION ([^\n]+)\n.*?% END VOL05-EXPANSION \1\n?",re.S)
EX=re.compile(r"% BEGIN VOL05-EXPANSION (V\d\d-example-\d\d)\n(.*?)% END VOL05-EXPANSION \1",re.S)
XB=re.compile(r"% BEGIN VOL05-EXPANSION (V\d\d-exercises-01)\n(.*?)% END VOL05-EXPANSION \1",re.S)
LAB=re.compile(r"\\label\{([^}]+)\}"); SECTION=re.compile(r"\\section\{([^}]+)\}")
def sha(t): return hashlib.sha256(t.encode()).hexdigest()
def cnt(t,e): return len(re.findall(rf"\\begin\{{{e}\}}",t))
def preceding(text,pos):
    ms=list(SECTION.finditer(text[:pos])); return ms[-1].group(1) if ms else ""
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--repo",required=True); ap.add_argument("--write",action="store_true"); a=ap.parse_args()
    repo=Path(a.repo).resolve(); basep=repo/REPORT/"VOLUME05_EXAMPLE_EXERCISE_BASELINE.json"; base=json.loads(basep.read_text()) if basep.exists() else {}; blockers=[]; rows=[]; labels=[]
    if not base: blockers.append("MISSING_COMMIT1_BASELINE")
    cats=[("standard","Standard computations",5),("proof","Proofs",4),("test","Counterexamples and hypothesis tests",3),("application","Applications and investigations",2),("challenge","Challenge problems",2)]
    fs=sorted((repo/VOL).glob("ch*/chapter.tex"))
    if len(fs)!=28: blockers.append(f"CHAPTER_COUNT:{len(fs)}!=28")
    for i,p in enumerate(fs,1):
        code=f"V/{i:02d}"; t=p.read_text(encoding="utf-8-sig"); labels += LAB.findall(t); ebs=list(EX.finditer(t)); xbs=list(XB.finditer(t)); placed=[]
        for b in ebs:
            sec=preceding(t,b.start()); placed.append(sec)
            if sec.lower().strip()=="worked examples": blockers.append(f"{code}:EXAMPLE_IN_COLLECTED_WORKED_SECTION")
        if len(ebs)!=3: blockers.append(f"{code}:EXPANSION_EXAMPLES:{len(ebs)}!=3")
        if len(xbs)!=1: blockers.append(f"{code}:EXERCISE_BLOCKS:{len(xbs)}!=1")
        body=xbs[0].group(2) if xbs else ""
        total=cnt(body,"exercise")
        if total!=16 or cnt(body,"hint")!=16 or cnt(body,"solution")!=16: blockers.append(f"{code}:TRIAD_COUNTS")
        positions=[]
        for key,heading,expected in cats:
            m=re.search(rf"\\subsection\*\{{{re.escape(heading)}\}}(.*?)(?=\\subsection\*\{{|$)",body,re.S)
            c=cnt(m.group(1),"exercise") if m else 0
            positions.append((key,c))
            if c!=expected: blockers.append(f"{code}:{key.upper()}:{c}!={expected}")
        old=next((x for x in base.get("chapters",[]) if x["chapter"]==code),None)
        protected=sha(BLOCK.sub("",t))
        if not old or old["protected_sha256"]!=protected: blockers.append(f"{code}:PROTECTED_TEXT_CHANGED_OR_MISSING")
        rows.append({"chapter":code,"placed_after":placed,"categories":dict(positions),"expansion_examples":len(ebs),"expansion_exercises":total,"protected_sha256":protected})
    for lab,n in Counter(labels).items():
        if n>1: blockers.append(f"DUPLICATE_LABEL:{lab}")
    status="PASS" if not blockers else "FAIL"; result={"status":status,"chapters":rows,"blocking":blockers}
    print(json.dumps({"status":status,"blocking":blockers},indent=2))
    if a.write:
        out=repo/REPORT; out.mkdir(parents=True,exist_ok=True)
        (out/"VOLUME05_EXAMPLE_EXERCISE_BALANCE_AUDIT.json").write_text(json.dumps(result,indent=2)+"\n")
        md=["# Volume V worked-example placement and graded-exercise balance","",f"**Result:** {status}","","| Chapter | New examples | New exercises | 5/4/3/2/2 |","|---|---:|---:|---|"]
        for r in rows:
            c=r["categories"]; md.append(f"| {r['chapter']} | {r['expansion_examples']} | {r['expansion_exercises']} | {c.get('standard')}/{c.get('proof')}/{c.get('test')}/{c.get('application')}/{c.get('challenge')} |")
        md += ["","## Placement evidence",""]
        for r in rows: md.append(f"- **{r['chapter']}**: {' | '.join(r['placed_after'])}")
        md += ["","## Blocking findings",""]+([f"- {b}" for b in blockers] if blockers else ["None."])
        (out/"VOLUME05_EXAMPLE_EXERCISE_BALANCE_AUDIT.md").write_text("\n".join(md)+"\n")
    return 0 if status=="PASS" else 12
if __name__=="__main__": raise SystemExit(main())
