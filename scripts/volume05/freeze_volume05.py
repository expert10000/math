...
def write_manifest(vol):
    ...
    (f/"VOLUME05_FREEZE_MANIFEST.sha256").write_text("\n".join(lines)+"\n",encoding="utf-8")

def check_manifest(vol):
    for line in (vol/"freeze/VOLUME05_FREEZE_MANIFEST.sha256").read_text(encoding="utf-8").splitlines():
...
def main():
    ...
    f=vol/"freeze";f.mkdir(parents=True,exist_ok=True)
    recon=json.loads((vol/"reconstruction/VOLUME05_FULL_RECONCILIATION.json").read_text(encoding="utf-8"))
    if recon.get("status")!="PASS" or recon.get("unresolved_count")!=0:
        raise RuntimeError("Volume V reconciliation is not PASS / 0 unresolved")

    if args.phase=="pre":
        readme=vol/"README.md";text=readme.read_text(encoding="utf-8-sig")
        text=re.sub(r"(?m)^\*\*Status:\*\*.*$","**Status:** Full 28-chapter reconstruction reconciled; release build pending.",text,count=1)
        readme.write_text(text.rstrip()+"\n",encoding="utf-8")
        (f/"RELEASE_VOLUME05.md").write_text(
        ...
        write_manifest(vol)
        print("VOLUME V PRE-FREEZE AUDIT PASSED")
        return 0
    ...
    rows=tsv(repo/"editorial/CHAPTER_STATUS.tsv");hits=0
    for r in rows:
        if re.fullmatch(r"V/\d{2}",r.get("chapter_code","")):
            r["status"]="FROZEN";r["next_action"]="COMPLETE";hits+=1
    if hits!=28:
        raise RuntimeError(f"Expected 28 Volume V rows, found {hits}")
    write_status(repo/"editorial/CHAPTER_STATUS.tsv",rows)

    readme=vol/"README.md";text=readme.read_text(encoding="utf-8-sig")
    text=re.sub(r"(?m)^\*\*Status:\*\*.*$","**Status:** FROZEN — Volume V Commutative Algebra and Homological Methods v1.0 release baseline.",text,count=1)
    readme.write_text(text.rstrip()+"\n",encoding="utf-8")

    b=pdf.read_bytes();pages=len(re.findall(rb"/Type\s*/Page(?!s)",b))
    report=[
        "# Volume V Freeze Report","",
    ...
    (f/"VOLUME05_FREEZE_REPORT.md").write_text("\n".join(report),encoding="utf-8")
    print("VOLUME V FREEZE FINALIZED")
    return 0
