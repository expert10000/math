...
THEME_ORDER = [
    ("Vector Spaces", "I/01--I/06"),
    ("Matrices and Operators", "I/07--I/12"),
    ("Euclidean and Hilbert-Space Geometry", "I/13--I/18"),
]
...
def clean_solution(raw: str) -> str:
    ...
    raw = re.sub(
        r"\\(?:chapter|section|subsection|subsubsection|paragraph)\*?\s*\{\s*(?:Solution|Answer)\s*\}",
        " ",
        raw,
        flags=re.I,
    )
    raw = re.sub(r"(?im)^\s*(?:\\textbf\{)?(?:Solution|Answer)\}?\s*[:.]\s*", "", raw)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Migrate Companion Part I problem corpus.")
    ...
    if not part:
        print("ERROR: no Volume I rows in Companion atlas")
        return 2
    ...
    for ar in sorted(part, key=lambda r: r.get("companion_problem_id","")):
        ...
        theme = ar.get("thematic_section","") or "Vector Spaces"
        related = ar.get("related_chapters","") or dict(THEME_ORDER).get(theme,"I/01--I/18")
    ...
    out = [
        r"\chapter{Problems Related to Volume I}",
        r"\label{ch:companion-i}",
        "",
        r"\paragraph{Main-text correspondence.}",
        r"This Part accompanies \emph{Volume I: Linear Algebra}. Problems are grouped by",
    ...
    for theme in ordered_themes:
        ...
        out += [
            ...
            "",
            rf"\paragraph{{Related material.}} Volume I, Chapters \texttt{{{related}}}.",
            "",
        ]
        for item in items:
            label = item["cp"].lower()
    ...
    summary = [
        ...
        f"- Atlas placement-review rows included in Part I: **{review_count}**",
        "",
    ...
    print("COMPANION PART I MIGRATION BUILT")
