...
def main():
    ...
    prereq=json.loads((reports/'VOL02_PREREQUISITE_AUDIT.json').read_text(encoding='utf-8'))
    hints=json.loads((reports/'VOL02_HINT_THEOREM_AUDIT.json').read_text(encoding='utf-8'))
    math=json.loads((reports/'VOL02_REMAINING_MATH_REPAIRS.json').read_text(encoding='utf-8'))
    blocking=[]
    if math.get('status')!='PASS': blocking.append('mathematical repair evidence is not PASS')
    ...
    if not latexmk:
        ...
        try:
            ...
            build={
                ...
                'pdf':'books/vol02_real_analysis/book.pdf',
                'pages':pages,
            ...
            build={'status':'FAIL','error':str(exc)}
            blocking.append('BUILD:'+str(exc))

    (reports/'VOL02_BUILD_AFTER_PRO_REVIEW.json').write_text(json.dumps(build,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    obj={
        'schema':1,
        'status':'PASS' if not blocking and build.get('status')=='PASS' else 'FAIL',
        'scope':'Volume II professional/mathematical review after v1.2',
        'chapters':25,
        'mathematical_repairs':math.get('status'),
        'prerequisite_notation_audit':prereq.get('status'),
        'exercise_hint_theorem_audit':hints.get('status'),
        'build':build,
        'blocking':blocking,
    }
    (reports/'VOL02_PROFESSIONAL_REVIEW_FINAL.json').write_text(json.dumps(obj,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    md=[
        '# Volume II — professional/mathematical review final','',
        f"**Status:** {obj['status']}",'',
        f"- Mathematical statement/proof repairs: **{math.get('status')}**",
        f"- II/01--II/25 prerequisite/notation audit: **{prereq.get('status')}**",
        f"- Exercise-hint/theorem coverage audit: **{hints.get('status')}**",
        f"- Clean Volume II build: **{build.get('status')}**",
    ...
    (reports/'VOL02_PROFESSIONAL_REVIEW_FINAL.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
