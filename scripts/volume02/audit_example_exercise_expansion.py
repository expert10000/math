...
def main():
    ...
    outdir=repo/REPORT
    if not a.check_only: outdir.mkdir(parents=True,exist_ok=True)
    baseline=outdir/"VOLUME02_EXAMPLE_EXERCISE_BASELINE.json"
    ...
    for i,p in enumerate(fs,1):
        ...
        row={"chapter":code,"path":str(p.relative_to(repo)).replace("\\","/"),
             "examples":cnt(t,"example"),"exercises":cnt(t,"exercise"),
             "hints":cnt(t,"hint"),"problems":cnt(t,"problem"),
             "solutions":cnt(t,"solution"),"labels":len(LABEL.findall(t)),
             "protected_sha256":sha(strip(t)),
             "expansion_blocks":len(re.findall(r"% BEGIN VOL02-EXPANSION ",t))}
        rows.append(row); labels += [(x,code) for x in LABEL.findall(t)]
        if row["exercises"]!=row["hints"]:
            blockers.append(f"{code}:EXERCISE_HINT_MISMATCH")
        if row["solutions"]<row["exercises"]+row["problems"]:
            blockers.append(f"{code}:TOO_FEW_SOLUTIONS")

        if not a.snapshot and base:
            old=next((x for x in base["chapters"] if x["chapter"]==code),None)
            if old and old["protected_sha256"]!=row["protected_sha256"]:
                blockers.append(f"{code}:PROTECTED_TEXT_CHANGED")
            if old and row["examples"]<old["examples"]:
                blockers.append(f"{code}:EXAMPLE_COUNT_DECREASE")
            if old and row["exercises"]<old["exercises"]:
    ...
    counts=Counter(x for x,_ in labels)
    for label,n in counts.items():
        if n>1:
            blockers.append(f"DUPLICATE_LABEL:{label}")

    status="PASS" if not blockers else "FAIL"
    result={"status":status,"stage":a.stage,"chapters":rows,"blocking":blockers}

    if a.snapshot and not a.check_only:
        baseline.write_text(json.dumps({"schema":1,"chapters":rows},indent=2)+"\n",encoding="utf-8")
    if not a.check_only:
        (outdir/"VOLUME02_EXAMPLE_EXERCISE_AUDIT.json").write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
        md=["# Volume II worked-example and graded-exercise audit","",f"**Result:** {status}","",
            "| Chapter | Examples | Exercises | Hints | Problems | Solutions | Expansion blocks |",
            "|---|---:|---:|---:|---:|---:|---:|"]
        for r in rows:
            md.append(f"| {r['chapter']} | {r['examples']} | {r['exercises']} | {r['hints']} | {r['problems']} | {r['solutions']} | {r['expansion_blocks']} |")
        md += ["","## Blocking findings",""]
        md += [f"- {b}" for b in blockers] if blockers else ["None."]
        (outdir/"VOLUME02_EXAMPLE_EXERCISE_AUDIT.md").write_text("\n".join(md)+"\n",encoding="utf-8")
