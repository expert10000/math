...
VOL=Path("books/vol06_algebraic_geometry/chapters")
...
def main():
    ...
    if len(fs)!=49:
        blockers.append(f"CHAPTER_COUNT:{len(fs)}!=49")
    if a.stage>1 and not base:
    ...
    for i,p in enumerate(fs,1):
        ...
        labs=LAB.findall(t)
        labels+=labs
        ebs=list(EB.finditer(t))
        xbs=list(XB.finditer(t))

        r={
            "chapter":code,
            "path":p.relative_to(repo).as_posix(),
            "examples":cnt(t,"example"),
            "exercises":cnt(t,"exercise"),
            "hints":cnt(t,"hint"),
            "problems":cnt(t,"problem"),
            "solutions":cnt(t,"solution"),
            "labels":len(labs),
            "protected_sha256":sha(strip(t)),
            "expansion_examples":len(ebs),
        ...
        # Existing Volume VI pedagogy is intentionally heterogeneous.
        # Record pre-existing total-count deficits, but do not make them blockers.
        if r["exercises"]!=r["hints"]:
            observations.append(
                f"{code}:LEGACY_EXERCISE_HINT_IMBALANCE:{r['exercises']}!={r['hints']}"
            )
        if r["solutions"] < r["exercises"] + r["problems"]:
            observations.append(
                f"{code}:LEGACY_SOLUTION_COVERAGE:"
                f"{r['solutions']}<{r['exercises']+r['problems']}"
            )
        ...
        enriched=(a.stage>=2 and i<=17) or (a.stage>=3 and 18<=i<=32)
        if enriched:
            if not old:
                blockers.append(f"{code}:MISSING_BASELINE")
            else:
                # Require exact additions relative to whatever baseline the chapter had.
                for key,delta in [
            ...
            # The new block itself must be perfectly balanced, irrespective of legacy counts.
            if r["expansion_examples"]!=3:
                blockers.append(
                    f"{code}:EXPANSION_EXAMPLES:{r['expansion_examples']}!=3"
                )
            if r["expansion_exercise_blocks"]!=1:
                blockers.append(
                    f"{code}:EXPANSION_EXERCISE_BLOCKS:"
                    f"{r['expansion_exercise_blocks']}!=1"
                )
            if r["expansion_exercises"]!=16:
                blockers.append(
                    f"{code}:EXPANSION_EXERCISES:{r['expansion_exercises']}!=16"
                )
            if r["expansion_hints"]!=16:
                blockers.append(
                    f"{code}:EXPANSION_HINTS:{r['expansion_hints']}!=16"
                )
            if r["expansion_solutions"]!=16:
                blockers.append(
                    f"{code}:EXPANSION_SOLUTIONS:{r['expansion_solutions']}!=16"
                )
        elif a.stage>=2:
            if r["expansion_examples"] or r["expansion_exercise_blocks"]:
    ...
    if not a.check_only:
            ...
            if any(r["expansion_examples"] or r["expansion_exercise_blocks"] for r in rows):
                ...
                bp.write_text(
                    json.dumps({
                        "schema":2,
                        "volume":"VI",
                        "policy":"heterogeneous-legacy-baseline",
                        "chapters":rows,
                        "baseline_observations":observations,
                    },indent=2)+"\n",
                    encoding="utf-8",
        ...
        (out/"VOLUME06_EXAMPLE_EXERCISE_AUDIT.json").write_text(
            json.dumps(result,indent=2)+"\n",encoding="utf-8"
        )
        md=[
            "# Volume VI worked-example and graded-exercise audit","",
            f"**Result:** {status}","",
            f"**Stage:** {a.stage}","",
        ...
        for r in rows:
            md.append(
                f"| {r['chapter']} | {r['examples']} | {r['exercises']} | "
                f"{r['hints']} | {r['problems']} | {r['solutions']} | "
                f"{r['expansion_examples']} | {r['expansion_exercises']} |"
            )
        md += ["","## Blocking findings",""]
        md += [f"- {b}" for b in blockers] if blockers else ["None."]
        md += ["","## Baseline observations (non-blocking)",""]
        md += [f"- {b}" for b in observations] if observations else ["None."]
