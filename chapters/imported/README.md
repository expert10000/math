# Imported chapter sources

This folder is the cleaned working copy promoted from `tex_test/`.

## Layout

```text
chapters/imported/
  tex/          # 158 deduplicated TeX files
...
```powershell
.\chapters\imported\compile-one.ps1 -File tex\theory-of-analysis.tex
```

Compile all imported TeX files:

```powershell
.\chapters\imported\compile-all.ps1
```

Generated PDFs and logs are written to `pdfs/imported/`, which is intentionally
ignored by git.
