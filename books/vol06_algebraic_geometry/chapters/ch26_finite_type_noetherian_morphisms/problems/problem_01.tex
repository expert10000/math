...
\begin{problem}[AG5-P4: every affine open of a locally Noetherian scheme is Noetherian]\label{prob:vi26-p01}
Let \(X\) be a scheme.  Prove the theorem that \(X\) is locally Noetherian if and only if every affine open \(U=\Spec A\subseteq X\) has \(A\) Noetherian.
\par\smallskip\begingroup\emergencystretch=2em\noindent\textit{context.} Recovered numbered Supplementary Problem AG5-P4; previously routed by VI-SCOPE-\allowbreak 041 but absent from the ledger.\par\endgroup
\end{problem}
