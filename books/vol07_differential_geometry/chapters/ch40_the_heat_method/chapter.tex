...
% VOL07-NUMERICAL-HEAT-SOLVER-DIAGNOSTICS-VII40
\begin{remark}[Residuals in the two heat-method linear solves]
The heat method contains two sparse solves, and each has its own algebraic
residual.  For the heat step,
\[
r_{\mathrm{heat}}
=
M\delta_s-(M+tL)\widetilde u,
\]
while after fixing the Poisson gauge,
\[
r_{\mathrm{pois}}
=
b-L\widetilde\phi.
\]
These residuals should be reported in a scale-aware norm, for example relative
to the corresponding right-hand side when it is nonzero.

A small linear-system residual certifies only that the computed vector nearly
solves the assembled discrete system.  It does not certify that the chosen
mesh, mass matrix, stiffness matrix, time \(t\), boundary condition, gradient
normalization, or source discretization accurately represents the intended
continuous geodesic-distance problem.

The full error budget is therefore layered:
\[
\begin{aligned}
&\text{geometric/mesh model}
+ \text{finite-element discretization}
+ \text{heat-time bias}\\
&\qquad
+ \text{linear-solver error}
+ \text{gradient/Poisson reconstruction error}.
\end{aligned}
\]
Solver tolerances should be tight enough that algebraic error is not the
dominant term, but making the residual arbitrarily small cannot remove the
other sources of error.  Benchmark distance and path accuracy independently of
the sparse-solver stopping criterion.
\end{remark}
...
Figure~\ref{fig:vii40-c16-repro-pipeline} and
Table~\ref{tab:vii40-c16-laplacian-conventions} fix the operator and API
conventions used by the rebuildable reference data.  The local benchmark in
Table~\ref{tab:vii40-c16-heat-convergence} records mesh scale, heat time,
error, boundary treatment, and deterministic work units; it is not a claim of
production sparse-solver timing.
\input{chapters/ch40_the_heat_method/c16_reproducibility.tex}
\input{../../code/volume07/results/heat_distance_convergence.tex}
...
Figure~\ref{fig:vii40-c07-heat-pipeline} and Table~\ref{tab:vii40-c07-heat-conventions}
organize the two solves and the sign, scale, gauge, and boundary choices they require.
\input{chapters/ch40_the_heat_method/c07_visuals}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Move the Laplacian to the left.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
An affine function has constant gradient.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Recall the heat-kernel asymptotic.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Diffusion length scales like square root of time.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
The heat remains extremely localized.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Heat every source simultaneously.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Mostly right-hand sides and local field operations.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Large eigenvalues are high frequency.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
The heat method relies on one central discrete operator.
\end{hint}
