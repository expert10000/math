...
% VOL07-NUMERICAL-GEODESIC-BENCHMARKING-VII38
\begin{remark}[Benchmark the metric that the algorithm actually solves]
A geodesic benchmark must name its reference problem.  The following are
different quantities:
\[
\begin{aligned}
d_E &\quad\text{(edge-graph distance)},\\
d_M &\quad\text{(exact polyhedral distance)},\\
d_{\mathrm{smooth}} &\quad\text{(distance on an intended smooth limit surface)}.
\end{aligned}
\]
Agreement with one of them does not imply agreement with the others.

For an approximate mesh solver, report at least a scalar distance error and,
when paths are recovered, a geometric path discrepancy.  Near a cut locus the
latter can be much more informative because distinct minimizing or nearly
minimizing paths may have almost equal length.  For convergence experiments,
refine the mesh while controlling not only edge length but also shape quality
and approximation of the intended metric.

A useful validation hierarchy is:
\[
\begin{aligned}
\text{analytic surface cases}
&\longrightarrow \text{exact polyhedral reference cases}\\
&\longrightarrow \text{mesh-refinement studies}
\longrightarrow \text{application meshes}.
\end{aligned}
\]
The solver type, source/target representation, boundary treatment, mesh scale,
and error norm are part of the benchmark specification.  Timing without an
accuracy target, or accuracy against an ill-defined reference metric, does not
support a meaningful algorithm comparison.
\end{remark}
...
Ask which data determine lengths after the mesh is bent in space.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Use an infimum over surface paths.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Compare any bent planar curve with its chord.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Flatten the two incident triangles into one plane.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Every graph path is also a surface path.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
An intrinsic path is not required to follow edges.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Compare the total angle with \(2\pi\).
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Look for more than one full turn of incident angle.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
The metric is not smooth there.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Specify both output types that may be requested.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Fix one source and vary the target.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
A source may lie in a face interior.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Use a level set of the distance function.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Think about uniqueness of minimizing paths.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Consider nearly equal competing geodesics.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Preserve the piecewise-Euclidean metric.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Think of conditioning and geometric reconstruction.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Examples include exactness and query model.
\end{hint}
...
\begin{hint}
% PEDAGOGY-ENRICHED-VII
Separate graph restriction from continuous face geometry.
\end{hint}
