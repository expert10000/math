...
\section{From Riemann sums to measure}
Riemann integration organizes the domain into finitely many intervals and asks whether upper and lower sums can be forced together. This is powerful for continuous and mildly discontinuous functions, but it treats a dense exceptional set as large whenever every interval meets it. The next volume changes the organizing question: instead of asking only how a function oscillates on intervals, it develops a notion of the \emph{size} of sets and integrates according to that size.

The conceptual transition is
\[
\boxed{
\begin{gathered}
\text{interval partitions}\\
\downarrow\\
\sigma\text{-algebras and measure}\\
\downarrow\\
\text{measurable functions}\\
\downarrow\\
\text{Lebesgue integral}\\
\downarrow\\
L^p\text{ and Fourier theory}.
\end{gathered}}
\]
Volume~III develops these steps systematically in Chapters~III/01--III/06.

\begin{example}[The Dirichlet function exposes the limitation]\label{ex:ii10-lebesgue-bridge-01}
Let
\[
f=\mathbf 1_{\mathbb Q\cap[0,1]}.
\]
Every interval contains both rational and irrational numbers, so every Darboux lower sum is \(0\) and every upper sum is \(1\). Hence \(f\) is not Riemann integrable.

Measure theory distinguishes this dense set from a genuinely large subset: the rationals are countable and will have Lebesgue measure zero. Once that machinery is available, the same function is Lebesgue integrable with integral \(0\). Density and measure-theoretic size are therefore different notions.
\end{example}

\begin{remark}[Null-set invariance preview]
Riemann integration already shows that changing finitely many values does not change an integral. Lebesgue integration extends this dramatically: changing an integrable function on a set of measure zero does not change its integral. This principle is one reason equivalence classes of functions, rather than pointwise formulas alone, become natural in \(L^p\) spaces.
\end{remark}

\begin{theorem}[Riemann--Lebesgue compatibility \proofstatus{used without full proof}]\label{thm:ii10-riemann-lebesgue-compat}
If a bounded function on \([a,b]\) is Riemann integrable, then it is Lebesgue integrable in the sense developed in Volume~III, and the two integrals have the same value.
\begin{proof}
The proof is deferred to Volume~III after measurable sets, null sets, and the Lebesgue integral have been constructed. One standard route uses the characterization of bounded Riemann-integrable functions by the measure-zero size of their discontinuity set.
\end{proof}
\end{theorem}

\section{Why the Lebesgue integral changes convergence arguments}
The Riemann theory in this volume handles limit--integral interchange through uniform convergence and related estimates. Measure theory adds convergence theorems that permit much more flexible pointwise limiting processes. Volume~III develops monotone convergence and dominated convergence and then uses them throughout \(L^p\), Fourier analysis, and PDE.

\begin{example}[A limit problem that motivates domination]\label{ex:ii10-lebesgue-bridge-02}
Suppose \(f_n(x)\to f(x)\) pointwise. Pointwise convergence alone does not justify
\[
\lim_{n\to\infty}\int f_n=\int f.
\]
In Chapter~12 we impose stronger classical hypotheses such as uniform convergence. Volume~III replaces that single route by measure-theoretic hypotheses---for example, domination by one integrable function---that can permit interchange even when convergence is not uniform.
\end{example}

\section{Bridge exercises to Volume III}
\begin{exercise}[Dense versus large]\label{exr:ii10-lebesgue-bridge-01}
Why does density of \(\mathbb Q\) force failure of Riemann integrability for \(\mathbf 1_{\mathbb Q}\), while countability points toward a different conclusion in measure theory?
\end{exercise}
\begin{solution}
Riemann upper and lower sums inspect every interval, and each interval sees both values \(0\) and \(1\). Measure theory instead assigns size to sets; countable subsets of \(\mathbb R\) have measure zero, so the exceptional value occurs on a null set.
\end{solution}

\begin{exercise}[Compatibility principle]\label{exr:ii10-lebesgue-bridge-02}
What should happen to the value of the integral when a bounded Riemann-integrable function is reinterpreted in Lebesgue theory?
\end{exercise}
\begin{solution}
The value is unchanged. Lebesgue integration extends the Riemann integral rather than replacing its values on the classical domain.
\end{solution}

\begin{exercise}[Why a new convergence theorem?]\label{exr:ii10-lebesgue-bridge-03}
Why is uniform convergence useful but restrictive for interchanging a limit and an integral?
\end{exercise}
\begin{solution}
It gives one error bound valid at every point, which makes integral control immediate, but many natural approximating sequences converge only pointwise or almost everywhere. Measure-theoretic convergence theorems are designed for those broader situations.
\end{solution}
...
\begin{theorem}[Monotone functions are Riemann integrable]\label{thm:ii10-04}
Every bounded monotone real-valued function on a compact interval $[a,b]$ is Riemann integrable.
\begin{proof}
Assume $f$ is increasing; the decreasing case is analogous. For the equal partition into $n$ subintervals of width $h=(b-a)/n$, the upper-lower gap is
\[
U(f,P)-L(f,P)=h\sum_{i=1}^n\bigl(f(x_i)-f(x_{i-1})\bigr)
=h\bigl(f(b)-f(a)\bigr).
\]
This tends to zero as $n\to\infty$, so the Darboux criterion applies.
\end{proof}
\end{theorem}
...
\begin{problem}[Integral of a step function]\label{prob:ii10-problem-01}
...
\begin{problem}[Darboux gap estimate]\label{prob:ii10-problem-02}
...
\begin{problem}[Continuous integrability mechanism]\label{prob:ii10-problem-03}
...
\begin{problem}[Monotone integrability]\label{prob:ii10-problem-04}
...
\begin{problem}[Integral of $x$]\label{prob:ii10-problem-05}
...
\begin{problem}[A single discontinuity]\label{prob:ii10-problem-06}
...
\begin{problem}[Dirichlet function]\label{prob:ii10-problem-07}
...
\begin{problem}[Absolute-value inequality]\label{prob:ii10-problem-08}
...
\begin{problem}[Mean value for integrals]\label{prob:ii10-problem-09}
...
\begin{problem}[FTC computation]\label{prob:ii10-problem-10}
...
\begin{problem}[Additivity]\label{prob:ii10-problem-11}
...
\begin{problem}[Integration synthesis]\label{prob:ii10-problem-12}
...
% VOL02-HINT-RECONCILIATION-II10
...
\end{exercise}
\begin{hint}
The integral of the constant 1 over [0,2] is the rectangle area base times height.
\end{hint}
\begin{solution}
It is $2$.
\end{solution}
...
\end{exercise}
\begin{hint}
Split the interval at c and use additivity: integrate separately on [a,c] and [c,b].
\end{hint}
\begin{solution}
