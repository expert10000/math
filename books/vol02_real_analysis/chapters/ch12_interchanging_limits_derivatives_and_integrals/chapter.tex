...
% VOL02-DEPENDENCY-II12-DIRECT-UNIFORM-TAIL
...
For $|x|\le r<1$, the geometric-series tail satisfies
\[
\sup_{|x|\le r}\left|\sum_{n>N}x^n\right|
\le\sum_{n>N}r^n=\frac{r^{N+1}}{1-r}\to0.
\]
Thus the series converges uniformly on $[-r,r]$, and the uniform-limit integral theorem gives
\[
\int_0^r\sum_{n\ge0}x^n\,dx
=\sum_{n\ge0}\frac{r^{n+1}}{n+1}
=-\log(1-r).
\]
Chapter~13 packages this kind of domination into the Weierstrass M-test.
\end{example}
...
\begin{problem}[Integral interchange estimate]\label{prob:ii12-problem-01}
...
\begin{problem}[A pointwise failure for integrals]\label{prob:ii12-problem-02}
...
\begin{problem}[Termwise integration of a geometric series]\label{prob:ii12-problem-03}
For $|x|\le r<1$, justify termwise integration of $\sum_{n\ge0}x^n$ without invoking the M-test from Chapter~13.
\end{problem}
\begin{solution}
The uniform tail estimate
\[
\sup_{|x|\le r}\left|\sum_{n>N}x^n\right|\le\frac{r^{N+1}}{1-r}
\]
tends to zero. Hence the series converges uniformly and may be integrated termwise by the theorem of this chapter.
\end{solution}

\begin{problem}[Derivative exchange mechanism]\label{prob:ii12-problem-04}
...
\begin{problem}[Derivative counterexample]\label{prob:ii12-problem-05}
...
\begin{problem}[Uniform derivative convergence]\label{prob:ii12-problem-06}
...
\begin{problem}[Series of derivatives]\label{prob:ii12-problem-07}
...
\begin{problem}[Local uniform convergence]\label{prob:ii12-problem-08}
...
\begin{problem}[Continuity of integration operator]\label{prob:ii12-problem-09}
...
\begin{problem}[Differentiation is not bounded in sup norm]\label{prob:ii12-problem-10}
...
\begin{problem}[Uniform-majorant preview]\label{prob:ii12-problem-11}
Why is a point-independent summable majorant useful before termwise integration?
\end{problem}
\begin{solution}
A summable majorant bounds every function-series tail by one numerical tail independent of the evaluation point, giving uniform convergence. Chapter~13 formalizes this principle as the Weierstrass M-test.
\end{solution}

\begin{problem}[Interchange synthesis]\label{prob:ii12-problem-12}
...
% VOL02-HINT-RECONCILIATION-II12
...
\end{exercise}
\begin{hint}
Derivative data determine a function only up to an additive constant; one convergent value fixes that constant.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Sup norm on a compact interval.
\end{hint}
\begin{solution}
The sup norm.
\end{solution}
