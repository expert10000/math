...
The inverse and implicit function principles are local nonlinear analogues of solving an invertible linear system. They provide the local solvability tools that connect several-variable differentiation with later fixed-point and differential-equation arguments.
...
\section{Preview: contraction proof strategy}
% VOL02-DEPENDENCY-PREVIEW-II09-CONTRACTION
One standard proof route reformulates the nonlinear equation as a contraction on a small ball. Banach's contraction mapping theorem is developed formally in Chapter~16; the inverse and implicit function statements in this chapter may be used independently of that later proof mechanism.

\section{Failure modes}
Singular derivative, noninjectivity, or rank loss can destroy local solvability or uniqueness.

% VOL02-NUMERICAL-INVERSE-II09
\begin{remark}[Invertibility versus numerical local solvability]
The inverse and implicit function theorems are exact theorems: their hypotheses
ask whether the derivative or the relevant derivative block is actually
invertible.  Numerical computation introduces a second question---how
sensitive the corresponding linear solve is.

If a square derivative \(J\) is invertible, then
\[
\|J^{-1}\|_2=\frac{1}{\sigma_{\min}(J)}.
\]
A very small positive \(\sigma_{\min}(J)\) therefore means that the inverse
derivative can strongly amplify perturbations even though the exact inverse
function theorem applies.  Conversely, if \(J\) is available only
approximately, declaring it singular or nonsingular requires a stated
tolerance and an error model; a floating-point test such as
\(\det J=0\) versus \(\det J\ne0\) is not a substitute for that analysis.

The same warning applies to the block \(D_yF\) in the implicit function
theorem.  Exact local existence and uniqueness, numerical conditioning, and
the reliability of a computed rank decision are related but distinct issues.
\end{remark}
...
\begin{remark}[Proof-status convention]
Advanced results whose arguments are intentionally abbreviated are marked in their theorem titles. \proofstatus{proof sketch} means that the proof mechanism is explained while some infrastructure is deferred; \proofstatus{used without full proof} means that the result is an explicit mathematical input rather than a theorem proved completely in this volume. For the inverse function theorem, Chapter~16 supplies the contraction theorem used by the proof route sketched below.
\end{remark}

\section{Core structural results}

\begin{theorem}[Inverse function theorem \proofstatus{proof sketch}]\label{thm:ii09-01}
...
\begin{proof}
Proof sketch. After composing with $Df(a)^{-1}$, normalize so that the derivative at $a$ is the identity. A standard proof then controls the nonlinear remainder on a sufficiently small ball and solves $f(x)=y$ by Banach's contraction theorem. Chapter~16 develops that fixed-point theorem and its quantitative estimates. Once local existence and uniqueness of the inverse are established, the differentiability of the inverse follows from the linearized equation and the chain rule.
\end{proof}
\end{theorem}
...
\begin{problem}[One-dimensional inverse theorem]\label{prob:ii09-problem-01}
...
\begin{problem}[A locally invertible map]\label{prob:ii09-problem-02}
...
\begin{problem}[Local versus global inverse]\label{prob:ii09-problem-03}
...
\begin{problem}[Implicit circle graph]\label{prob:ii09-problem-04}
...
\begin{problem}[Implicit derivative formula]\label{prob:ii09-problem-05}
...
\begin{problem}[Regular level curve]\label{prob:ii09-problem-06}
...
\begin{problem}[A singular failure]\label{prob:ii09-problem-07}
...
\begin{problem}[Coordinate straightening]\label{prob:ii09-problem-08}
...
\begin{problem}[Newton linearization link]\label{prob:ii09-problem-09}
...
\begin{problem}[Parameter dependence]\label{prob:ii09-problem-10}
...
\begin{problem}[Rank perspective]\label{prob:ii09-problem-11}
...
\begin{problem}[IFT synthesis]\label{prob:ii09-problem-12}
...
% VOL02-HINT-RECONCILIATION-II09
...
\end{exercise}
\begin{hint}
The inverse theorem gives a local diffeomorphism, and a diffeomorphism maps a sufficiently small neighborhood onto a neighborhood.
\end{hint}
\begin{solution}
...
\(y''(0)=-1\).
\end{solution}
\begin{exercise}[Exact invertibility and numerical rank]\label{exr:ii09-numerical-conditioning-01}
Suppose an exact derivative matrix is
\[
J=
\begin{pmatrix}
1&0\\
...
\end{pmatrix}.
\]
Compute \(\kappa_2(J)\).  Using the relative numerical-rank rule
\[
...
Explain why the exact inverse function theorem and the numerical rank decision
are answering different questions.
\end{exercise}
\begin{hint}
The singular values are the absolute diagonal entries.  Exact invertibility
asks whether the smaller one is exactly zero; the numerical rule compares it
with the chosen threshold.
\end{hint}
\begin{solution}
The singular values are \(1\) and \(10^{-10}\), hence
\[
\kappa_2(J)=10^{10}.
\]
For \(\tau=10^{-12}\), both singular values exceed the threshold, so
\(\operatorname{rank}_{10^{-12}}J=2\).  For \(\tau=10^{-8}\), the smaller
singular value is discarded and
\(\operatorname{rank}_{10^{-8}}J=1\).

As an exact matrix, \(J\) is invertible because \(10^{-10}\ne0\), so the exact
inverse function theorem can apply if its other hypotheses hold.  Numerically,
however, the inverse is highly sensitive and a tolerance may classify the
small direction as unresolved.  A computed Jacobian would require an error
bound before either numerical classification could be promoted to an exact
claim about the true derivative.
\end{solution}
