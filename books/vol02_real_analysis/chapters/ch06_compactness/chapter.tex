...
% VOL02-TOTAL-BOUNDED-DEFINITION
A metric space is \emph{totally bounded} if for every $\varepsilon>0$ it can be covered by finitely many balls of radius $\varepsilon$. Compact metric spaces are exactly the complete, totally bounded metric spaces; a formal theorem and proof are recorded below in the theorem-coverage reconciliation.
...
\begin{theorem}[Heine--Borel theorem]\label{thm:ii06-04}
A subset of $\mathbb R^n$ is compact if and only if it is closed and bounded.
\begin{proof}
Compact subsets of metric spaces are closed and bounded. Conversely, a bounded sequence in a closed bounded subset of $\mathbb R^n$ has a convergent subsequence by Bolzano--Weierstrass, and closedness keeps its limit in the set. Thus the set is sequentially compact; in metric spaces sequential compactness is equivalent to compactness.
\end{proof}
\end{theorem}

\begin{theorem}[Compact metric criterion]\label{thm:ii06-05}
A metric space is compact if and only if it is complete and totally bounded.
\begin{proof}
Compactness gives total boundedness by applying a finite subcover to the cover by $\varepsilon$-balls. It gives completeness because every Cauchy sequence has a convergent subsequence and the Cauchy property forces the whole sequence to the same limit. Conversely, total boundedness lets one choose a Cauchy subsequence from every sequence by repeatedly passing to one ball in a finite $2^{-k}$-cover containing infinitely many remaining terms. Completeness supplies its limit. Hence the space is sequentially compact, and therefore compact in the metric setting.
\end{proof}
\end{theorem}
...
\begin{problem}[Open interval is not compact]\label{prob:ii06-problem-01}
...
\begin{problem}[Sequential noncompactness]\label{prob:ii06-problem-02}
...
\begin{problem}[Compact sets are closed]\label{prob:ii06-problem-03}
...
\begin{problem}[Compact sets are bounded]\label{prob:ii06-problem-04}
...
\begin{problem}[Continuous real functions are bounded]\label{prob:ii06-problem-05}
...
\begin{problem}[Positive distance between disjoint compact sets]\label{prob:ii06-problem-06}
Prove nonempty disjoint compact sets $K,L$ in a metric space have positive distance.
\end{problem}
\begin{solution}
The product $K\times L$ is compact, and the continuous function $(x,y)\mapsto d(x,y)$ attains a minimum there. If the minimum were zero, some $x\in K$ and $y\in L$ would satisfy $d(x,y)=0$, hence $x=y$, contradicting disjointness. Therefore $d(K,L)>0$.
\end{solution}

\begin{problem}[Finite intersection property]\label{prob:ii06-problem-07}
...
\begin{problem}[Nested compact sets]\label{prob:ii06-problem-08}
...
\begin{problem}[Heine--Cantor contradiction sequence]\label{prob:ii06-problem-09}
...
\begin{problem}[Bolzano--Weierstrass by nested boxes]\label{prob:ii06-problem-10}
...
\begin{problem}[Closed subset of compact is compact]\label{prob:ii06-problem-11}
Prove it by covers.
\end{problem}
\begin{solution}
...
\begin{problem}[Compactness synthesis]\label{prob:ii06-problem-12}
...
% VOL02-HINT-RECONCILIATION-II06
...
\end{exercise}
\begin{hint}
From an arbitrary open cover choose one member containing each of the finitely many points.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
A Cauchy sequence in a compact metric space has a convergent subsequence; use the Cauchy property to show the whole sequence has the same limit.
\end{hint}
\begin{solution}
Yes.
\end{solution}
