# Volume II — Real Analysis and Topological Foundations

**Status:** Canonical reconstruction underway; II/01–II/07 developed with solved dossiers.

Canonical chapter codes: `II/01`–`II/25`.

## Part I — Metric Foundations

- **II/01** — The Real Number System and Completeness
- **II/02** — Euclidean and Normed Spaces
- **II/03** — Sequences and Cauchy Sequences
- **II/04** — Open and Closed Sets
- **II/05** — Metric Spaces and Continuity
- **II/06** — Compactness
- **II/07** — Connectedness and Path Connectedness

## Part II — Calculus

- **II/08** — Differentiability in Several Variables
- **II/09** — Inverse and Implicit Function Principles
- **II/10** — Riemann Integration

## Part III — Sequences of Functions

- **II/11** — Pointwise and Uniform Convergence
- **II/12** — Interchanging Limits, Derivatives and Integrals
- **II/13** — Infinite Series of Functions
- **II/14** — Trigonometric Series
- **II/15** — Pathological and Nowhere-Differentiable Functions

## Part IV — Fixed Points and Differential Equations

- **II/16** — Contraction Mappings
- **II/17** — Brouwer-Type Fixed-Point Ideas
- **II/18** — Integral Equations
- **II/19** — Elementary Existence Theory for ODEs

## Part V — Approximation

- **II/20** — Polynomial Interpolation
- **II/21** — Polynomial Approximation
- **II/22** — Chebyshev and Minimax Approximation
- **II/23** — The Alternation Principle
- **II/24** — Numerical Quadrature
- **II/25** — Continued Fractions and Approximation Topics

## Reconstruction rule

Before creating a chapter, filter `../../editorial/SOURCE_MIGRATION.tsv` by its `II/NN` destination and inspect all higher-precedence overrides. Update `../../editorial/CHAPTER_STATUS.tsv` when work starts.
