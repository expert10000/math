...
\section{Continuity is part of the definition}
Linearity alone is not enough. A distribution
\(T:\mathcal D(\Omega)\to\mathbb C\) must be continuous in the topology of
Chapter~15. Equivalently, on every fixed compact support \(K\Subset\Omega\)
there are \(C_K>0\) and an integer \(m_K\ge0\) for which
\[
\boxed{
|T(\varphi)|
\le C_K
...
\qquad
(\varphi\in\mathcal D_K(\Omega)).
\]
The integer \(m_K\) is a local order bound. It may depend on \(K\); a general
distribution need not have one finite order valid on all of \(\Omega\).

\begin{example}[Continuity estimates for regular distributions and point masses]
If \(f\in L^1_{\mathrm{loc}}(\Omega)\), then for
\(\operatorname{supp}\varphi\subset K\),
\[
|\langle T_f,\varphi\rangle|
=
\left|\int_K f\varphi\right|
\le
\|f\|_{L^1(K)}\,p_0(\varphi).
\]
Thus every regular distribution has local order zero.

For \(a\in\Omega\),
\[
|\langle\delta_a,\varphi\rangle|
=|\varphi(a)|\le p_0(\varphi)
\]
whenever \(a\in K\), and the pairing is zero when \(a\notin K\). Hence
\(\delta_a\) also has order zero. Its derivative satisfies
\[
...
\le p_{|\alpha|}(\varphi),
\]
showing explicitly how distributional differentiation raises the local order
bound.
\end{example}
...
\begin{theorem}[Local finite-order characterization]\label{thm:iii16-04}
A linear functional \(T\) on \(C_c^\infty(\Omega)\) is a distribution if and
only if, for every compact \(K\Subset\Omega\), its restriction to
\(\mathcal D_K(\Omega)\) satisfies a finite-order estimate
\[
|T(\varphi)|\le C_K p_{m_K}(\varphi).
\]
\begin{proof}
This is the continuity criterion of
Theorem~\ref{thm:iii15-04}, applied to the definition of a distribution as a
continuous linear functional on \(\mathcal D(\Omega)\).
\end{proof}
\end{theorem}
...
\begin{problem}[Dirac mass is a continuous distribution]\label{prob:iii16-problem-01}
...
\begin{problem}[Regular distributions from locally integrable functions]\label{prob:iii16-problem-02}
...
\begin{problem}[The derivative of Heaviside is Dirac]\label{prob:iii16-problem-03}
...
\begin{problem}[A piecewise smooth jump creates a delta]\label{prob:iii16-problem-04}
...
\begin{problem}[Derivative of Dirac]\label{prob:iii16-problem-05}
...
\begin{problem}[Distributional derivative definition]\label{prob:iii16-problem-06}
...
\begin{problem}[Classical compatibility]\label{prob:iii16-problem-07}
...
\begin{problem}[Continuity is essential]\label{prob:iii16-problem-08}
...
\begin{problem}[Agreement with classical derivatives]\label{prob:iii16-problem-09}
...
\begin{problem}[Derivative of Heaviside]\label{prob:iii16-problem-10}
...
\begin{problem}[Continuity of differentiation]\label{prob:iii16-problem-11}
...
\begin{problem}[Chapter synthesis]\label{prob:iii16-problem-12}
