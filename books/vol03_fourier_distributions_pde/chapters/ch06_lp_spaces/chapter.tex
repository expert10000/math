...
$L^p(X,\mu)$ consists of a.e. classes of measurable functions with finite displayed norm. Before quotienting, the expression is a seminorm. Unless stated otherwise $1\le p\le\infty$; power-integral computations, Chebyshev estimates and truncation convergence use $p<\infty$. The $p=\infty$ case uses essential bounds directly. No sigma-finiteness is needed for completeness or simple-function density in finite $p$: for a given $f\in L^p$, the sets $\{|f|>1/j\}$ have finite measure and cover its nonzero set up to null sets.
...
\begin{proof}
Choose a subsequence whose increments $h_j$ satisfy $\|h_j\|_p\le2^{-j}$. For finite $p$, Minkowski and MCT give $\|\sum_j|h_j|\|_p\le\sum_j2^{-j}$, so the series converges a.e. and its norm tails tend to zero. For $p=\infty$, remove the countable union of exceptional null sets; the series then converges uniformly with tail bound $\sum_{j\ge N}2^{-j}$. The subsequence converges in norm in both cases, and the Cauchy property gives convergence of the full sequence.
\end{proof}
\end{theorem}

\section{Worked examples}

\begin{example}[Functions equal a.e. represent the same \(L^p\) element]\label{ex:iii06-worked-01}
On \([0,1]\), let \(f=0\) and let \(g=\mathbf1_{\{1/2\}}\). Then
\(f=g\) a.e. and
\[
\|f-g\|_p=0
\]
for every \(1\le p<\infty\). Thus \(L^p\) consists of equivalence classes, not
pointwise functions.
\end{example}

\begin{example}[Finite measure gives \(L^q\subset L^p\)]\label{ex:iii06-worked-02}
If \(\mu(X)<\infty\) and \(1\le p<q<\infty\), Hölder gives
\[
\|f\|_p\le \mu(X)^{1/p-1/q}\|f\|_q.
\]
Thus stronger integrability implies weaker integrability on finite-measure
spaces, with the measure factor showing exactly where finiteness enters.
\end{example}

\begin{example}[Essential supremum ignores null spikes]\label{ex:iii06-worked-03}
Let \(f=1\) a.e. on \([0,1]\) but set \(f(1/2)=10^6\). Then
\[
\|f\|_\infty=1
\]
in the essential-supremum sense. A single null spike does not change the
\(L^\infty\) class.
\end{example}

\begin{example}[A power singularity test]\label{ex:iii06-worked-04}
For \(f(x)=x^{-\alpha}\) on \((0,1)\),
\[
f\in L^p(0,1)\quad\Longleftrightarrow\quad \alpha p<1,
\]
because \(\int_0^1x^{-\alpha p}dx\) is finite exactly under that inequality.
This gives a sharp membership threshold.
\end{example}
...
\begin{problem}[Functions equal a.e. represent the same \(L^p\) element]\label{prob:iii06-problem-01}
Give two distinct pointwise functions that represent the same \(L^p[0,1]\) element.
\end{problem}
\begin{solution}
Take \(0\) and \(\mathbf1_{\{1/2\}}\); they differ only on a null set, so
their \(L^p\) distance is zero.
\end{solution}

\begin{problem}[Finite measure gives \(L^q\subset L^p\)]\label{prob:iii06-problem-02}
Derive the finite-measure embedding \(L^q(X)\subset L^p(X)\) for \(p<q\).
\end{problem}
\begin{solution}
Apply Hölder to \(|f|^p\cdot1\) with conjugate exponents \(q/p\) and
\(q/(q-p)\), then take \(p\)-th roots.
\end{solution}

\begin{problem}[Essential supremum ignores null spikes]\label{prob:iii06-problem-03}
Explain why changing a bounded measurable function at one point does not change its \(L^\infty\) norm.
\end{problem}
\begin{solution}
The \(L^\infty\) norm is the essential supremum, which ignores sets of
measure zero.
\end{solution}

\begin{problem}[A power singularity test]\label{prob:iii06-problem-04}
Determine when \(x^{-\alpha}\) belongs to \(L^p(0,1)\).
\end{problem}
\begin{solution}
Exactly when \(\alpha p<1\), by the elementary convergence test for
\(\int_0^1x^{-\alpha p}dx\).
\end{solution}

\begin{problem}[Norm zero means zero a.e.]\label{prob:iii06-problem-05}
If \(\|f\|_p=0\), prove \(f=0\) a.e.
\end{problem}
\begin{solution}
The nonnegative function \(|f|^p\) has integral zero, hence vanishes a.e.
\end{solution}

\begin{problem}[An infinite-measure failure of inclusion]\label{prob:iii06-problem-06}
Show \(L^q(\mathbb R)\subset L^p(\mathbb R)\) need not hold for \(p<q\).
\end{problem}
\begin{solution}
Choose a power-decay function whose \(q\)-power is integrable at infinity
but whose \(p\)-power is not, e.g. tune \(f(x)=(1+|x|)^{-\alpha}\).
\end{solution}

\begin{problem}[Completeness meaning]\label{prob:iii06-problem-07}
What does completeness of \(L^p\) say about a Cauchy sequence?
\end{problem}
\begin{solution}
Every \(L^p\)-Cauchy sequence converges in \(L^p\) to an \(L^p\) equivalence
class; the limit need not be pointwise visible from the definition alone.
\end{solution}

\begin{problem}[Probability-space comparison]\label{prob:iii06-problem-08}
If \(\mu(X)=1\) and \(p<q\), simplify the finite-measure norm comparison.
\end{problem}
\begin{solution}
The measure factor is \(1\), so \(\|f\|_p\le\|f\|_q\).
\end{solution}

\begin{problem}[Norm-zero criterion]\label{prob:iii06-problem-09}
...
\begin{problem}[Finite-measure inclusion]\label{prob:iii06-problem-10}
...
\begin{problem}[Completeness of $L^p$]\label{prob:iii06-problem-11}
...
\end{problem}
\begin{solution}
Choose a subsequence whose increments $h_j$ satisfy $\|h_j\|_p\le2^{-j}$. For finite $p$, Minkowski and MCT give $\|\sum_j|h_j|\|_p\le\sum_j2^{-j}$, so the series converges a.e. and its norm tails tend to zero. For $p=\infty$, remove the countable union of exceptional null sets; the series then converges uniformly with tail bound $\sum_{j\ge N}2^{-j}$. The subsequence converges in norm in both cases, and the Cauchy property gives convergence of the full sequence. This proof also explains why the stated hypothesis is structural rather than cosmetic.
\end{solution}

\begin{problem}[Chapter synthesis]\label{prob:iii06-problem-12}
...
For which p in the range 1<=p<=infinity is x\textasciicircum{}(-1/3) in Lp(0,1)?
\end{exercise}
\begin{hint}
Require p/3<1.
\end{hint}
...
\end{hint}
\begin{solution}
M\textasciicircum{}(1/p) for finite p.
\end{solution}
...
\end{hint}
\begin{solution}
At most 2\textasciicircum{}p/t\textasciicircum{}p.
\end{solution}
...
\end{exercise}
\begin{hint}
Apply the zero-integral criterion to |f|\textasciicircum{}p.
\end{hint}
\begin{solution}
For finite $p$, apply the zero-integral criterion to $|f|^p$. For $p=\infty$, the sets $\{|f|>1/j\}$ are null for every $j$, so their union $\{|f|>0\}$ is null.
\end{solution}
...
\end{exercise}
\begin{hint}
Apply Holder to |f|\textasciicircum{}p times 1.
\end{hint}
\begin{solution}
...
\end{hint}
\begin{solution}
Bad-set measure is bounded by norm error to the p divided by epsilon\textasciicircum{}p.
\end{solution}
...
\end{hint}
\begin{solution}
The tail indicator tends to zero under the integrable dominator |f|\textasciicircum{}p.
\end{solution}
...
\end{hint}
\begin{solution}
For $1\le p<q\le\infty$, take $f(x)=x^{-1/p}$ on $[1,\infty)$. Then $\int |f|^p=\infty$, while $f\in L^q$ (bounded if $q=\infty$, integrable to the $q$th power otherwise).
\end{solution}
...
\end{hint}
\begin{solution}
For $1\le p<\infty$, enumerate the indicators of the dyadic half-open intervals $[j2^{-k},(j+1)2^{-k})$, $0\le j<2^k$, level by level on $[0,1)$. Their norms are $2^{-k/p}\to0$, but every point sees infinitely many zeros and ones. For $p=\infty$, norm convergence does imply uniform convergence outside one null set for a sequence of representatives.
\end{solution}
...
If f is in L1 and Linfinity, prove ||f||\_p\textasciicircum{}p <= ||f||\_infinity\textasciicircum{}(p-1)||f||\_1.
\end{exercise}
\begin{hint}
Factor |f|\textasciicircum{}p.
\end{hint}
\begin{solution}
