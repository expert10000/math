...
All dilation formulas assume integrable $f$ and nonzero real dilation. In $n$ dimensions, $\widehat{f(a\,\cdot)}(\xi)=|a|^{-n}\widehat f(\xi/a)$. For a real symmetric positive-definite matrix $A$, the Gaussian transform is $(\det A)^{-1/2}\exp(-\pi\xi^TA^{-1}\xi)$ for $\exp(-\pi x^TAx)$. Heat kernels are $G_t(x)=(4\pi t)^{-n/2}\exp(-|x|^2/(4t))$ for $t>0$; their mass is one and $\widehat G_t=e^{-4\pi^2t|\xi|^2}$. Semigroup formulas for functions require $s,t>0$; at time zero the kernel is $\delta_0$.
...
Derivatives of Gaussians are polynomial times Gaussian, and this polynomial-Gaussian space is Fourier invariant. The orthonormal Hermite eigenbasis requires appropriately scaled Hermite functions; arbitrary successive derivatives of one Gaussian are not asserted to be an orthogonal eigenbasis.
...
\begin{example}[The normalized Gaussian is self-dual]\label{ex:iii12-worked-01}
Under the \(2\pi\)-frequency convention,
\[
g(x)=e^{-\pi x^2}
\quad\Longrightarrow\quad
\widehat g(\xi)=e^{-\pi\xi^2}.
\]
One proof differentiates the transform, obtains an ODE
\(\widehat g'=-2\pi\xi\,\widehat g\), and uses
\(\widehat g(0)=\int e^{-\pi x^2}dx=1\).
\end{example}

\begin{example}[Gaussian scaling]\label{ex:iii12-worked-02}
For \(a>0\),
\[
f_a(x)=e^{-\pi a x^2}
\]
...
\]
This follows from Gaussian self-duality plus the transform scaling law.
\end{example}

\begin{example}[Heat-kernel scaling]\label{ex:iii12-worked-03}
The function
\[
K_t(x)=(4\pi t)^{-n/2}e^{-|x|^2/(4t)}
\]
has total mass \(1\) and the parabolic scaling
\[
K_t(x)=t^{-n/2}K_1(x/\sqrt t).
\]
Its Fourier transform is \(e^{-4\pi^2t|\xi|^2}\), matching the heat multiplier
under the declared convention.
\end{example}

\begin{example}[Moments become frequency derivatives]\label{ex:iii12-worked-04}
If \(xf(x)\in L^1\), differentiation under the integral gives
\[
\partial_\xi\widehat f(\xi)
=-2\pi i\,\widehat{xf}(\xi).
\]
Thus spatial moments control regularity of the transform.
\end{example}
...
\begin{problem}[The normalized Gaussian is self-dual]\label{prob:iii12-problem-01}
Show that \(e^{-\pi x^2}\) is fixed by the Fourier transform under the volume convention.
\end{problem}
\begin{solution}
Differentiate the transform to obtain the Gaussian ODE and use the
normalization at zero; the unique solution is \(e^{-\pi\xi^2}\).
\end{solution}

\begin{problem}[Gaussian scaling]\label{prob:iii12-problem-02}
Compute the transform of \(e^{-\pi a x^2}\) for \(a>0\).
\end{problem}
\begin{solution}
Apply the scaling rule to the self-dual Gaussian, giving
\(a^{-1/2}e^{-\pi\xi^2/a}\).
\end{solution}

\begin{problem}[Heat-kernel scaling]\label{prob:iii12-problem-03}
Verify the scaling and transform multiplier of the heat kernel.
\end{problem}
\begin{solution}
Parabolic scaling is direct. Gaussian transform scaling gives
\(\widehat K_t(\xi)=e^{-4\pi^2t|\xi|^2}\).
\end{solution}

\begin{problem}[Moments become frequency derivatives]\label{prob:iii12-problem-04}
Derive the relation between \(\partial_\xi\widehat f\) and \(\widehat{xf}\).
\end{problem}
\begin{solution}
Differentiate the exponential inside the transform integral:
\(\partial_\xi e^{-2\pi ix\xi}=-2\pi ix e^{-2\pi ix\xi}\).
\end{solution}

\begin{problem}[Gaussian integral]\label{prob:iii12-problem-05}
State the normalization \(\int_{\mathbb R}e^{-\pi x^2}dx\).
\end{problem}
\begin{solution}
It equals \(1\). This normalization is what makes \(e^{-\pi x^2}\)
self-dual under the \(2\pi\)-frequency convention.
\end{solution}

\begin{problem}[Repeated derivatives]\label{prob:iii12-problem-06}
Why does a Gaussian times a polynomial remain rapidly decaying?
\end{problem}
\begin{solution}
Each derivative of a Gaussian is a polynomial times the same Gaussian, and
Gaussian decay dominates every polynomial.
\end{solution}

\begin{problem}[Heat semigroup multiplier]\label{prob:iii12-problem-07}
What is the product of the heat multipliers at times \(s\) and \(t\)?
\end{problem}
\begin{solution}
\(e^{-4\pi^2s|\xi|^2}e^{-4\pi^2t|\xi|^2}
=e^{-4\pi^2(s+t)|\xi|^2}\), reflecting the semigroup law.
\end{solution}

\begin{problem}[Frequency width]\label{prob:iii12-problem-08}
Qualitatively, what happens to the Fourier width when a Gaussian is narrowed in space?
\end{problem}
\begin{solution}
The reciprocal scaling law broadens it in frequency, illustrating the
space-frequency tradeoff.
\end{solution}

\begin{problem}[Gaussian transform]\label{prob:iii12-problem-09}
...
\begin{problem}[Scaling rule]\label{prob:iii12-problem-10}
...
\begin{problem}[Heat-kernel semigroup]\label{prob:iii12-problem-11}
...
\begin{problem}[Chapter synthesis]\label{prob:iii12-problem-12}
...
\end{hint}
\begin{solution}
Derivatives of Gaussians are polynomial times Gaussian, and this polynomial-Gaussian space is Fourier invariant. The orthonormal Hermite eigenbasis requires appropriately scaled Hermite functions; arbitrary successive derivatives of one Gaussian are not asserted to be an orthogonal eigenbasis. As an immediate consequence, the same principle can be inserted into later convergence, approximation, transform, or weak-form arguments whenever its hypotheses are verified.
\end{solution}
...
\end{hint}
\begin{solution}
Integral of exp(-pi x\textasciicircum{}2) is 1.
\end{solution}

\begin{exercise}[Self transform]\label{exr:iii12-expand-02}
What is the transform of exp(-pi x\textasciicircum{}2)?
\end{exercise}
\begin{hint}
Use self-reciprocity.
\end{hint}
\begin{solution}
It is exp(-pi xi\textasciicircum{}2).
\end{solution}

\begin{exercise}[Scaled Gaussian]\label{exr:iii12-expand-03}
Transform exp(-4 pi x\textasciicircum{}2).
\end{exercise}
\begin{hint}
Use a=4.
\end{hint}
\begin{solution}
One half times exp(-pi xi\textasciicircum{}2/4).
\end{solution}

\begin{exercise}[Derivative]\label{exr:iii12-expand-04}
Differentiate exp(-pi x\textasciicircum{}2).
\end{exercise}
\begin{hint}
Use chain rule.
\end{hint}
\begin{solution}
-2 pi x exp(-pi x\textasciicircum{}2).
\end{solution}
...
\end{hint}
\begin{solution}
It gives a\textasciicircum{}(-1/2) G(xi/sqrt(a)).
\end{solution}
...
How can differentiating in a generate transforms involving x\textasciicircum{}2 times a Gaussian?
\end{exercise}
\begin{hint}
Differentiate the Gaussian family before and after transform.
\end{hint}
\begin{solution}
The a derivative inserts a factor proportional to -x\textasciicircum{}2, generating the transform identity.
\end{solution}
