...
If a distribution tail is bounded by 4 t^(-2), which weak space is indicated?
\end{exercise}
\begin{hint}
Match the exponent.
\end{hint}
...
\end{hint}
\begin{solution}
For every epsilon, the measure of |f_n-f|>epsilon tends to zero.
\end{solution}
...
\end{hint}
\begin{solution}
Its tail has critical t^(-p) decay while its p-th power integral diverges.
\end{solution}
...
Compute the weak-Lp tail quantity for x^(-1/p) on (0,1).
\end{exercise}
\begin{hint}
Handle t<1 and t>=1.
\end{hint}
\begin{solution}
The supremum of t^p times the superlevel measure is 1.
\end{solution}
