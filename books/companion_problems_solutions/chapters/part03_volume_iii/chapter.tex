...
\begin{solution}
Each assertion is an instance of the Baire-category definitions.

Since \(\mathbb Q\) is countable and each singleton is nowhere dense in
\(\mathbb R\),
\[
\mathbb Q=\bigcup_{q\in\mathbb Q}\{q\}
\]
is meager; hence \(\mathbb R\setminus\mathbb Q\) is comeager.  The
Liouville numbers form a dense \(G_\delta\), so they are comeager in
\(\mathbb R\), even though they have Lebesgue measure \(0\).  A Cantor set
has empty interior and is closed, hence is nowhere dense; therefore both
the standard Cantor set and a fat Cantor set are meager.  This also shows
that category and measure are independent notions: a fat Cantor set may
have positive measure while remaining meager.

In \(C([0,1])\) with the sup norm, the cited genericity statements are
Baire-category statements: the exceptional regularity classes are
meager, while the complementary properties are comeager.

Finally, if \(E_1,E_2,\ldots\) is any prescribed countable family of
finite-dimensional subspaces of an infinite-dimensional Banach space,
then each \(E_n\) is closed and has empty interior, hence is nowhere
dense.  Therefore
\[
...
The word ``any'' in the source must be read in this countable-family
sense: literally taking the union of all finite-dimensional subspaces
would give the whole space, because every vector lies in its
...
\qquad (s,t\ge0).
\]
Apply this to the normalized quantities
\[
s=\frac{|f(x)|}{\|f\|_p},
\qquad
t=\frac{|g(x)|}{\|g\|_q}.
...
\[
\int
\frac{|f(x)g(x)|}{\|f\|_p\|g\|_q}\,dx
\le
\frac1p+\frac1q=1,
\]
hence
\[
\boxed{\int |fg|\le \|f\|_p\|g\|_q}.
\]

Minkowski's integral inequality follows by applying the dual form of the
\(L^p\)-norm.  If
\[
G(y)=\int F(x,y)\,dx,
\]
...
\int |G(y)|\,|g(y)|\,dy
&\le
\iint |F(x,y)|\,|g(y)|\,dy\,dx\\
&\le
\int
\left(\int |F(x,y)|^p\,dy\right)^{1/p}dx.
\end{aligned}
\]
...
\boxed{
\left(
\int\left|\int F(x,y)\,dx\right|^pdy
\right)^{1/p}
\le
\int
\left(\int |F(x,y)|^pdy\right)^{1/p}dx.
}
\]

For the concrete indicator example in the problem, both sides equal
\(1\), so equality holds.
\end{solution}
...
\par\medskip\noindent\textbf{Deep Theory / Roadmap}\quad
...
\par\smallskip\noindent\textbf{Part (a): Minkowski's inequality for $p=1$}\quad
...
\par\smallskip\noindent\textbf{Additional examples for part (a)}\quad
...
\par\smallskip\noindent\textbf{Example 1: Gaussian.}\quad
...
\par\smallskip\noindent\textbf{Example 2: Polynomial times Gaussian.}\quad
Let
\[
...
\par\smallskip\noindent\textbf{Example 3: Trigonometric example.}\quad
On $[0,\pi]$ let
\[
...
\par\medskip\noindent\textbf{How simple functions and regular functions are related}\quad
...
\par\smallskip\noindent\textbf{(ii) Young's inequality as convex duality.}\quad
...
\par\smallskip\noindent\textbf{Young's inequality and concavity of $\log$.}\quad
...
\par\smallskip\noindent\textbf{Legendre-transform viewpoint.}\quad
...
\par\smallskip\noindent\textbf{(iii) Hölder's inequality.}\quad
...
\par\smallskip\noindent\textbf{(iv) Minkowski's inequality and $L^p$--norms.}\quad
...
\par\smallskip\noindent\textbf{(v) Simple functions and general measurable functions.}\quad
...
\begin{solution}
The reduction from simple functions to general functions is an
approximation argument.

For a nonnegative measurable \(F\), choose simple functions
\[
0\le F_1\le F_2\le\cdots,\qquad F_m\uparrow F.
\]
Minkowski's integral inequality is immediate for each finite simple
function because all integrals reduce to finite sums.  Passing to the
limit uses monotone convergence.

For a general complex-valued \(F\), apply the estimate to \(|F|\).  The
pointwise triangle inequality gives
\[
\left|\int F(x,y)\,dx\right|
\le
\int |F(x,y)|\,dx,
\]
so the nonnegative case controls the general case.

Equivalently, for \(1\le p<\infty\), let
\[
G(y)=\int F(x,y)\,dx.
\]
Using the dual representation of the \(L^p\)-norm and Hölder in the
\(y\)-variable,
\[
\begin{aligned}
\|G\|_{L^p_y}
...
\[
\boxed{
\left\|\int F(x,\cdot)\,dx\right\|_{L^p}
\le
\int\|F(x,\cdot)\|_{L^p}\,dx.
}
\]
Density of simple functions supplies the rigorous passage from the
finite-sum case to the full \(L^p\) statement.
\end{solution}
...
(f,g)
=
\frac1{(2\pi)^n}(\widehat f,\widehat g),
\qquad
\|\widehat f\|_2=(2\pi)^{n/2}\|f\|_2.
\]
It is proved first for Schwartz functions by Fourier inversion and
Fubini, then extended to \(L^2\) by density.

Applying the transform twice gives
\[
\boxed{\mathcal F^2f=(2\pi)^n\check f},
\qquad
\check f(x)=f(-x),
\]
hence \(\mathcal F\) is bijective on \(L^2\) and
\[
\mathcal F^{-1}h(x)
=
\frac1{(2\pi)^n}\mathcal F[h](-x).
\]
...
f_R=\mathbf1_{B_R}f,
\]
we have \(f_R\to f\) in \(L^2\), so Plancherel yields
\[
\mathcal F[f_R]\to\mathcal F[f]
\quad\text{in }L^2.
\]
This is exactly the asserted truncated-integral formula.

If \(f,D_jf\in L^2\), approximation by Schwartz functions and integration
by parts give
\[
\boxed{\mathcal F[D_jf](\xi)=i\xi_j\widehat f(\xi)},
\]
and therefore \(\xi_j\widehat f\in L^2\).

For \(f(x)=\sin x/x\), the function is bounded near \(0\) and
\(O(|x|^{-1})\) at infinity, hence \(f\in L^2(\mathbb R)\).  Under the
stated transform convention,
\[
\boxed{
\widehat f(\xi)=
\begin{cases}
...
\end{cases}}
\]
with boundary values irrelevant in \(L^2\).
...
\|f\|_2\|g\|_2.
\]
Translation continuity in \(L^2\) implies \(f*g\) is continuous, and
Schwartz approximation gives
\[
\boxed{f*g=\mathcal F^{-1}(\widehat f\,\widehat g)}.
\]
...
\quad(n=3),
\]
polar coordinates show \(G\in L^1\), and the radial Fourier-transform
formula yields
\[
\boxed{\widehat G(\xi)=\frac1{|\xi|^2+k^2}}.
\]
\end{solution}
...
\begin{solution}
The Plancherel identity corresponding to the stated convention is
\[
\boxed{
(f,g)=\frac1{(2\pi)^n}
...
\]
Thus part (a) of the migrated statement contains a typographical slip:
its second inner-product entry should be \(\mathcal Ff\), not a second
copy of \(\mathcal Fg\).

For Schwartz functions this follows directly from Fourier inversion and
Fubini; density then extends it to \(L^2\).  Applying the transform twice,
\[
\mathcal F^2f=(2\pi)^n\check f,
\]
so \(\mathcal F\) is bijective on \(L^2\) and its inverse is bounded.

For
\[
f_R=\mathbf1_{B_R}f,
\]
dominated convergence gives \(f_R\to f\) in \(L^2\).  Plancherel then
gives
\[
\mathcal F[f_R]\to\mathcal F[f]
\quad\text{in }L^2,
\]
which is the required truncated integral formula.

If \(f,D_jf\in L^2\), approximate by Schwartz functions.  Integration by
parts in the Schwartz class gives
\[
\mathcal F[D_jf](\xi)=i\xi_j\widehat f(\xi),
\]
and passage to the \(L^2\)-limit proves the same identity for \(f\).

For \(f(x)=\sin x/x\), boundedness near \(0\) and \(O(|x|^{-1})\) decay
give \(f\in L^2\), and
\[
\widehat f(\xi)=\pi\,\mathbf1_{(-1,1)}(\xi)
\]
up to irrelevant endpoint values.
...
\|f\|_2\|g\|_2
\]
by Cauchy--Schwarz.  Translation continuity in \(L^2\) gives continuity
of \(f*g\), while the convolution theorem, first on Schwartz functions
and then by approximation, yields
\[
\boxed{
f*g=\mathcal F^{-1}(\widehat f\,\widehat g).
}
...
Let $\alpha$ be a multiindex with $|\alpha| = k$.
...
% ------------------------------------------------------------
\par\medskip\noindent\textbf{Theory and Solutions}\quad

\par\smallskip\noindent\textbf{Sobolev norm.}\quad
For $s\ge 0$,
\[
...
% ------------------ (b) ---------------------
\par\smallskip\noindent\textbf{(b) \; Approximation by Compactly Supported Smooth Functions}\quad
...
% ------------------ (c) ---------------------
\par\smallskip\noindent\textbf{(c) \; Monotonicity of Sobolev Norms and $L^2$ Estimate}\quad
...
\par\smallskip\noindent\textbf{\textbf{$L^2$ estimate.}}\quad
Since
\[
\|f\|_{L^2}^2
...
% ------------------ (d) ---------------------
\par\smallskip\noindent\textbf{(d) \; Boundedness of Derivatives}\quad

\par\smallskip\noindent\textbf{\textbf{Goal.}}\quad
Show $D^\alpha : H^{s+k} \to H^{s}$ is bounded.

\par\smallskip\noindent\textbf{\textbf{Fourier representation.}}\quad
\[
\widehat{D^\alpha f}(\xi) = (i\xi)^\alpha \widehat{f}(\xi).
\]

\par\smallskip\noindent\textbf{\textbf{Estimate.}}\quad
...
\par\medskip\noindent\textbf{5.9 \quad Heat equation: energy estimate and heat kernel}\quad
...
\par\smallskip\noindent\textbf{\textbf{(a) Problem.}}\quad
Show that
\[
...
\par\smallskip\noindent\textbf{\textbf{(b) Problem.}}\quad
Show that
\[
u(t,x) = (u_0 * K_t)(x),
\]
...
\par\smallskip\noindent\textbf{\textbf{(c) Problem.}}\quad
...
\bigskip
\par\smallskip\noindent\textbf{\textbf{Theory.}}\quad
...
\bigskip
\par\smallskip\noindent\textbf{Solution to (a): \(L^2\)-energy estimate}\quad

\par\smallskip\noindent\textbf{\textbf{Step 1: Fourier representation.}}\quad
...
\par\smallskip\noindent\textbf{\textbf{Step 2: Use Plancherel.}}\quad
...
\bigskip
\par\smallskip\noindent\textbf{Solution to (b): convolution with the heat kernel}\quad

\par\smallskip\noindent\textbf{\textbf{Step 1: Gaussian heat kernel.}}\quad
Define
\[
K_t(x)
:= \frac{1}{(4\pi t)^{n/2}} \exp\!\left(-\frac{|x|^2}{4t}\right),
...
\par\smallskip\noindent\textbf{\textbf{Step 2: Convolution theorem.}}\quad
...
\bigskip
\par\smallskip\noindent\textbf{Solution to (c): positivity and conservation of mass}\quad

Assume \(u_0 \ge 0\).

\par\smallskip\noindent\textbf{\textbf{Positivity.}}\quad
From part (b),
\[
...
\par\smallskip\noindent\textbf{\textbf{Conservation of the \(L^1\)-norm.}}\quad
...
\par\medskip\noindent\textbf{Exercise 5.10 : The Schrödinger Equation in $\mathbb{R}^n$}\quad
...
\begin{solution}
Under the convention in the problem, Plancherel is
\[
(f,g)=\frac1{(2\pi)^n}(\widehat f,\widehat g).
\]
Accordingly, the displayed formula in part (a) should contain
\((\mathcal Ff,\mathcal Fg)\); two copies of \(\mathcal Fg\) would not
equal \((f,g)\) in general.

The remaining conclusions follow from the standard \(L^2\) extension of
the Fourier transform:
\[
\boxed{\mathcal F^2f=(2\pi)^n\check f},
\]
so \(\mathcal F\) is a bounded bijection of \(L^2\) with bounded inverse.
If \(f_R=\mathbf1_{B_R}f\), then \(f_R\to f\) in \(L^2\), and hence
\[
\mathcal F[f_R]\to\mathcal F[f]
\quad\text{in }L^2.
\]
For \(f,D_jf\in L^2\), Schwartz approximation yields
\[
\boxed{\mathcal F[D_jf]=i\xi_j\widehat f}.
\]
...
f\in L^2(\mathbb R),
\qquad
\widehat f=\pi\,\mathbf1_{(-1,1)}
\]
up to the endpoint convention.  Finally,
\[
|f*g(x)|\le\|f\|_2\|g\|_2
\]
and translation continuity in \(L^2\) gives \(f*g\in C^0\), while
\[
\boxed{
f*g=\mathcal F^{-1}(\widehat f\,\widehat g).
}
...
These identities also explain why the Schwartz space is the natural
dense core: \(\mathcal S\subset L^2\), \(\mathcal S\) is dense in
\(L^2\), and the Fourier transform is an automorphism of
\(\mathcal S\) and of \(\mathcal S'\).
\end{solution}
...
\[
\widehat g(\xi)=\int_{\mathbb R}e^{-2\pi ix\xi}g(x)\,dx.
\]
Transforming
\[
-\phi''+\phi=f
...
gives
\[
(1+4\pi^2\xi^2)\widehat\phi(\xi)=\widehat f(\xi),
\]
hence
\[
\boxed{
...
\]
The multiplier is smooth and all of its derivatives have at most
polynomial growth/decay of the type required to map
\(\mathcal S\) continuously into itself.  Therefore \(f\in\mathcal S\)
implies \(\phi\in\mathcal S\).
...
and
\[
\widehat G(\xi)=\frac1{1+4\pi^2\xi^2}.
\]
Thus
\[
\widehat\phi=\widehat f\,\widehat G
=
\widehat{f*G},
\]
so
...
\]
The homogeneous equation has solutions \(Ce^x+De^{-x}\), none of which
belongs to \(\mathcal S\) unless \(C=D=0\); hence the Schwartz solution is
unique.

The later radial-Fourier examples in the problem body use the same
principle: after Fourier transform, constant-coefficient elliptic
operators become multiplication by their symbols, and the Green kernel
is obtained by inverting that symbol.
\end{solution}
...
\begin{solution}
The figure sequence illustrates three standard convergence mechanisms.

For the weighted Dirac comb with
\[
c_g=(1+|g|)^2,
\]
and \(\varphi\in\mathcal S\), rapid decay gives, for every \(M\),
\[
|\varphi(2\pi g)|
\le
C_M(1+|g|)^{-M}.
...
C_M(1+|g|)^{2-M},
\]
and choosing \(M\) large enough makes the lattice sum absolutely
convergent.  Hence
\[
\sum_g c_g\delta_{2\pi g}
\]
defines a tempered distribution.  This is the estimate visualized in the
first two figures.

For the periodic \(L^p\) example, periodic convolution with a smooth
approximate identity produces smooth periodic functions
\(f_\varepsilon\) converging to \(f\) in \(L^p\) for \(1\le p<\infty\).

For the sawtooth, the Fourier partial sums converge in \(L^2\), and away
from jump points they converge pointwise to the usual Fourier limit.  Near
a jump, the localized overshoot persists as the number of modes grows;
this is the Gibbs phenomenon shown in the final figure.

Thus the four figures are consistent with the analytic statements they
are intended to illustrate: rapid Schwartz decay defeats polynomial
coefficient growth, mollification smooths periodic data, and Fourier
partial sums exhibit \(L^2\) convergence together with Gibbs oscillation
at jumps.
\end{solution}
...
Suppose that $\lambda = \{\lambda_1,\dots,\lambda_n\}$ is a basis for
	...
	\par\smallskip\noindent\textbf{(c) Dual lattice.}\quad
	...
	\par\smallskip\noindent\textbf{(d) Periodicity of exponential characters.}\quad
	...
	\par\smallskip\noindent\textbf{(e) Fourier transform of a $\Lambda$–periodic distribution.}\quad
	...
	\par\smallskip\noindent\textbf{(f) Fourier series representation.}\quad
...
Throughout, $\lambda=\{\lambda_1,\dots,\lambda_n\}$ is a basis of
	...
	\par\smallskip\noindent\textbf{(a) Construction of $\psi$.}\quad
	...
	\par\smallskip\noindent\textbf{(b) Mean value is independent of $\psi$.}\quad
	...
	\par\smallskip\noindent\textbf{(c) The dual lattice and dual basis.}\quad
	...
	\par\smallskip\noindent\textbf{(d) Periodicity of $e_g$.}\quad
	...
	\par\smallskip\noindent\textbf{(f) Fourier series representation and coefficients.}\quad
	...
	\par\medskip\noindent\textbf{Visual Summary for Exercise 5.7}\quad
...
\begin{solution}
For a compactly supported \(L^1\) function with
\[
\operatorname{supp}f\subset B_R(0),
\]
differentiation under the Fourier integral gives
\[
D_\xi^\alpha\widehat f(\xi)
=
\int_{B_R(0)}
(-ix)^\alpha f(x)e^{-ix\cdot\xi}\,dx.
\]
Therefore
\[
\boxed{
\sup_\xi|D_\xi^\alpha\widehat f(\xi)|
\le
R^{|\alpha|}\|f\|_1.
}
\]
This proves the estimate illustrated by the interval, bump, and ball
examples in the problem body.

For
\[
u_\alpha(x)=|x|^{-\alpha},
\]
polar coordinates show
\[
\int_{|x|<1}|x|^{-\alpha}\,dx
=
|S^{n-1}|
\int_0^1 r^{n-1-\alpha}\,dr,
\]
which is finite exactly when
\[
\boxed{\alpha<n}.
\]
Thus \(u_\alpha\) is a regular distribution in that range; at and beyond
the critical exponent one needs a principal-value or finite-part
extension.

For the one-dimensional Dirichlet Green kernel
\[
G(x,y)=
\begin{cases}
\dfrac{(x+1)(1-y)}2,&x\le y,\\[2mm]
\dfrac{(y+1)(1-x)}2,&x\ge y,
\end{cases}
\]
one has
\[
-\partial_{xx}G(\cdot,y)=\delta_y,
\qquad
G(\pm1,y)=0.
\]
Consequently, for
...
u(x)=
\begin{cases}
\dfrac{1+x}{2},&-1\le x<-\frac12,\\[2mm]
-\dfrac{x^2}{2}+\dfrac38,&|x|\le\frac12,\\[2mm]
\dfrac{1-x}{2},&\frac12<x\le1.
\end{cases}}
\]
Hence \(u\in H_0^1(-1,1)\) and
\[
-u''=\mathbf1_{[-1/2,1/2]}
\]
in the distributional sense.

These three calculations supply the common analytic mechanism behind the
long example block: compact support controls Fourier derivatives,
singular powers are classified by local integrability, and Green kernels
invert differential operators distributionally.
\end{solution}
...
\[
\int\phi_\varepsilon(x)\,dx
=
\int\phi(y)\,dy
=
1.
\]
The support shrinks according to
\[
\operatorname{supp}\phi_\varepsilon
=
\varepsilon\,\operatorname{supp}\phi.
\]
For every test function \(\psi\in\mathcal D(\mathbb R^n)\),
\[
\begin{aligned}
\langle\phi_\varepsilon,\psi\rangle
&=
\int
\varepsilon^{-n}\phi(x/\varepsilon)\psi(x)\,dx\\
&=
\int
...
Thus
\[
\boxed{\phi_\varepsilon\to\delta_0\text{ in }\mathcal D'.}
\]
...
\eta_\varepsilon(x)
=
(2\pi\varepsilon^2)^{-n/2}
e^{-|x|^2/(2\varepsilon^2)}
\]
has the same unit-mass and concentration properties, but it is not
compactly supported.  Hence
\[
\eta_\varepsilon\notin\mathcal D,
\qquad
...
-\Delta G=\delta_0,
\]
so, formally and distributionally,
\[
|k|^2\widehat G(k)=1,
\qquad
\widehat G(k)=\frac1{|k|^2}.
\]
Taking the inverse Fourier transform yields the classical fundamental
solution
\[
\boxed{
G(x)=
\begin{cases}
\dfrac{1}{(n-2)\omega_n}|x|^{2-n},&n\ge3,\\[3mm]
-\dfrac1{2\pi}\log|x|,&n=2,
\end{cases}}
\]
where \(\omega_n=|S^{n-1}|\) under the convention used in the problem.

Away from \(x=0\), these functions are harmonic.  Integrating
\(-\Delta G\) over a small ball and using the divergence theorem gives
unit flux through the boundary sphere, which verifies
\[
-\Delta G=\delta_0
\]
distributionally.
