...
\paragraph{Related material.} Volume II, Chapters \texttt{II/01--II/07}.
...
\hfill$\square$
	...
	\textbf{Quick examples.}
		...
		\par\noindent (b)\quad $f(x)=(x_1,x_2),\quad g(x)=(x_1^2,x_2^2)$ on $\mathbb{R}^2$.\\
		\[
		...
		\par\noindent (c)\quad $f(x_1,x_2)=\big(x_1^2+x_2,\ \sin x_1\big),\quad g(y_1,y_2)=y_1y_2$.\\
		\[
		Df\big|_{x_0}=
		\begin{pmatrix}
			2x_{01} & 1\\[2pt]
...
\paragraph{Related material.} Volume II, Chapters \texttt{II/08--II/10}.
...
\paragraph{Related material.} Volume II, Chapters \texttt{II/11--II/15}.
...
so $f_n\to 0$ uniformly. Hence for any $x_n\to x\ge 0$ we have $f_n(x_n)\to 0$.
	...
	\par\noindent\textbullet\quad The continuity of $f$ at the limit point is used twice: in Lemma~\ref{lem:move} and to conclude $f(x_k)\to f(x)$ in (ii).
	\par\noindent\textbullet\quad On compact domains, Remark~\ref{rem:equiv} provides a practical test for uniform convergence: \emph{check moving points}.
\end{problem}
...
{A dense set with super-polynomial rational approximation}{liouville-dense}
...
\paragraph{Related material.} Volume II, Chapters \texttt{II/16--II/19}.
...
\paragraph{Related material.} Volume II, Chapters \texttt{II/20--II/25}.
