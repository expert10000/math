...
% VOL01-INNER-PRODUCT-CONVENTION
\begin{remark}[Complex inner-product convention]
We use the standing complex-inner-product and star convention from
\hyperref[sec:i-notation-and-conventions]{Notation and Conventions}. It is
repeated here only where the conjugation order enters the calculation:
\[
...
\quad\text{on }\mathbb C^n.
\]
Thus \(A^*=\overline A^{\,T}\) for complex matrices. Chapter~16 uses the
same convention for operator adjoints.
\end{remark}

\begin{remark}[Series bridge: metrics and Riemannian geometry]
An inner product produces the norm \(\|v\|=\sqrt{\langle v,v\rangle}\), hence
a metric; this links directly to \emph{Volume II, Chapter 5: Metric Spaces and
Continuity}. The geometric continuation is
\emph{Volume VII, Chapter 20: Riemannian Metrics}, where each tangent space is
equipped with an inner product that varies smoothly with the base point.
Gram--Schmidt in Chapter~14 then connects to frame geometry in
\emph{Volume VII, Chapter 8}. See
...
\begin{theorem}[Finite-dimensional orthogonal decomposition]\label{thm:i13-03}
...
\]
\begin{proof}
Let \(u_1,\ldots,u_k\) be a basis of \(U\) and define
\[
\Phi:V\to\mathbb F^k,
\qquad
\Phi(v)=(\langle v,u_1\rangle,\ldots,\langle v,u_k\rangle).
\]
This map is linear because our inner product is linear in the first argument,
and \(\ker\Phi=U^\perp\). Restrict \(\Phi\) to \(U\). In the basis
\(u_1,\ldots,u_k\), its matrix is the Gram matrix
\(G=(\langle u_j,u_i\rangle)_{i,j}\). If \(Gc=0\) and
\(u=\sum_jc_ju_j\), then \(u\) is orthogonal to every \(u_i\), hence to
itself; positive definiteness gives \(u=0\), so \(c=0\). Thus \(G\) is
invertible and \(\Phi\) has rank \(k\). Rank--nullity gives
\(\dim U^\perp=\dim V-k\). Since \(U\cap U^\perp=\{0\}\), the dimensions
force \(V=U\oplus U^\perp\).
\end{proof}
\end{theorem}
...
\begin{problem}[Cauchy–Schwarz equality]\label{prob:i13-problem-01}
...
\begin{problem}[Orthogonal independence]\label{prob:i13-problem-02}
...
\begin{problem}[Weighted inner product]\label{prob:i13-problem-03}
...
\begin{problem}[Orthogonal complement]\label{prob:i13-problem-04}
...
\begin{problem}[Pythagoras]\label{prob:i13-problem-05}
...
\begin{problem}[Triangle inequality]\label{prob:i13-problem-06}
...
\begin{problem}[Complex conjugation placement]\label{prob:i13-problem-07}
...
\begin{problem}[Distance to a subspace]\label{prob:i13-problem-08}
...
\begin{problem}[Polarization]\label{prob:i13-problem-09}
...
\begin{problem}[Angle computation]\label{prob:i13-problem-10}
...
\begin{problem}[Orthogonal decomposition dimension]\label{prob:i13-problem-11}
If $U$ is a subspace of finite-dimensional $V$, prove $\dim U+\dim U^\perp=\dim V$.
\end{problem}
\begin{solution}
This is the dimension statement in the finite-dimensional orthogonal decomposition theorem proved above: applying rank--nullity to the Gram-coordinate map $v\mapsto(\langle v,u_i\rangle)_i$ gives $\dim U^\perp=\dim V-\dim U$.
\end{solution}

\begin{problem}[Inner-product synthesis]\label{prob:i13-problem-12}
...
\end{exercise}
\begin{hint}
A vector \((a,b)\) is orthogonal to every vector on the \(x\)-axis exactly when its first coordinate is zero.
\end{hint}
\begin{solution}
It is the $y$-axis.
\end{solution}
...
\end{exercise}
\begin{hint}
Expand both squared norms and cancel the cross terms.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
If \(v\in U\cap U^\perp\), test \(\langle v,v\rangle\).
\end{hint}
\begin{solution}
