...
\begin{remark}[Complex inner-product and star convention]
Throughout Volume~I, complex inner products are \emph{linear in the first
argument} and conjugate-linear in the second. Thus on \(\mathbb C^n\),
\[
\langle x,y\rangle=y^*x.
\]
For matrices, \(A^*=\overline A^{\,T}\) denotes the conjugate transpose
(and equals \(A^T\) over \(\mathbb R\)). Chapter~16 introduces the adjoint
operator characterized by \(\langle Tv,w\rangle=\langle v,T^*w\rangle\).
\end{remark}
