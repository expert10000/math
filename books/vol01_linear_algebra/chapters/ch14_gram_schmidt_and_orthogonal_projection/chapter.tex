...
% VOL01-COMPUTATIONAL-SPINE-I14
\begin{remark}[Computational spine III: choose coordinates that preserve geometry]
The basis machinery of Chapters~\ref{ch:i04-coordinates-and-change-of-basis}
and~\ref{ch:i07-matrix-representation-of-linear-maps} becomes especially
effective when the basis is orthonormal.  If
\[
Q=[q_1\ \cdots\ q_r],\qquad Q^*Q=I_r,
\]
then \(Q^*v\) is the coordinate vector of the orthogonal projection of \(v\)
onto the column space in the orthonormal basis \(q_1,\ldots,q_r\), and
\[
P_{\operatorname{col}Q}v=QQ^*v.
\]
So \(Q^*\) extracts orthonormal coordinates and \(Q\) reconstructs the
projected vector.

For a full-column-rank matrix \(A=QR\), least squares becomes
\[
\min_x\|Ax-b\|
=
\min_x\|Rx-Q^*b\|,
...
Rx=Q^*b.
\]
This is the coordinate-change viewpoint behind QR: first move to
norm-preserving orthonormal coordinates, then solve the triangular problem.
It also explains why forming \(A^*A\) is usually inferior numerically.

Chapter~\ref{ch:i18-singular-value-decomposition} completes this progression:
QR orthonormalizes the column geometry, whereas the SVD simultaneously chooses
orthonormal coordinates in the domain and codomain and exposes every stretching
factor explicitly.
\end{remark}
...
% VOL01-NUMERICAL-QR-RESIDUALS
\begin{remark}[Verifying a computed QR factorization]
For an exact reduced QR factorization \(A=QR\), the columns of \(Q\) are
orthonormal, so \(Q^*Q=I\).  A floating-point computation instead produces
approximations \(\widehat Q,\widehat R\).  For \(A\neq0\), two complementary
diagnostics are
\[
\rho_{QR}
=
\frac{\|A-\widehat Q\widehat R\|_2}{\|A\|_2},
\qquad
\eta_Q
=
\|\widehat Q^*\widehat Q-I\|_2.
\]
The relative residual \(\rho_{QR}\) tests how accurately the computed factors
reconstruct the supplied matrix.  The orthogonality defect \(\eta_Q\) tests
whether the computed columns of \(\widehat Q\) remain close to orthonormal.

These tests answer different questions.  A small reconstruction residual does
not by itself show that \(\widehat Q\) is nearly orthogonal, and a small
orthogonality defect does not show that \(\widehat Q\widehat R\) is close to
\(A\).  Even when both are small, they verify the computed factorization only
to the reported tolerance; they do not prove an exact rank, uniqueness of the
factors, or good conditioning of a downstream least-squares solution.
\end{remark}
...
% VOL01-DEPENDENCY-PREVIEW-I14-NUMERICS
\begin{remark}[Numerical preview]
The geometric least-squares theory is developed here. Condition numbers,
singular values and the Moore--Penrose pseudoinverse are treated systematically
in Chapter~18; numerical comparisons in this chapter are previews of that SVD
viewpoint.
\end{remark}
...
\begin{problem}[Gram–Schmidt by hand]\label{prob:i14-problem-01}
...
\begin{problem}[Projection onto a line]\label{prob:i14-problem-02}
...
\begin{problem}[Projection matrix]\label{prob:i14-problem-03}
...
\begin{problem}[Best approximation]\label{prob:i14-problem-04}
...
\begin{problem}[QR factorization]\label{prob:i14-problem-05}
...
\begin{problem}[Normal equations]\label{prob:i14-problem-06}
...
\begin{problem}[Why QR is stabler]\label{prob:i14-problem-07}
...
\begin{problem}[Projection idempotence]\label{prob:i14-problem-08}
...
\begin{problem}[Complementary projection]\label{prob:i14-problem-09}
...
\begin{problem}[Orthonormal coordinates]\label{prob:i14-problem-10}
...
\begin{problem}[Rank-deficient least squares]\label{prob:i14-problem-11}
What changes if the columns of $A$ are dependent?
\end{problem}
\begin{solution}
The least-squares projection onto the column space still exists uniquely, but the coefficient vector $x$ need not be unique because nonzero kernel vectors can be added without changing $Ax$. \textbf{Preview.} Chapter~18 introduces the pseudoinverse and shows that it selects the minimum-norm solution.
\end{solution}

\begin{problem}[Projection synthesis]\label{prob:i14-problem-12}
...
\end{exercise}
\begin{hint}
In reduced QR with full column rank, \(Q\) has one orthonormal column for each column of \(A\), while \(R\) is square \(n\times n\).
\end{hint}
\begin{solution}
...
\end{hint}
\begin{solution}
All are \(x_0+z\) with \(z\in\ker A\); they produce the same \(Ax\). \textbf{Preview.} Chapter~18 introduces the pseudoinverse and shows that it selects the minimum-norm member.
\end{solution}
\begin{exercise}[Residuals for a computed QR factorization]\label{exr:i14-numerical-qr-01}
Let
\[
A=I_2,\qquad
\widehat Q=
...
\|\widehat Q^*\widehat Q-I\|_2.
\]
Explain what each residual tests and why a zero reconstruction residual does
not prove that the computed \(Q\)-factor is orthogonal.
\end{exercise}
\begin{hint}
First multiply \(\widehat Q\widehat R\).  Then note that
\(\widehat Q^*\widehat Q-I\) is diagonal, so its spectral norm is the largest
absolute diagonal entry.
\end{hint}
\begin{solution}
The factors reconstruct the matrix exactly in this model:
\[
\widehat Q\widehat R=I_2=A,
\]
hence \(\rho_{QR}=0\).  However,
\[
\widehat Q^*\widehat Q-I
=
\begin{pmatrix}
0&0\\
0&(1+\varepsilon)^2-1
...
so
\[
\eta_Q=(1+\varepsilon)^2-1
=2\varepsilon+\varepsilon^2
=2.000001\times10^{-6}.
\]
Thus \(\rho_{QR}\) tests reconstruction of \(A\), whereas \(\eta_Q\) tests the
orthonormality structure required of \(Q\).  The example has perfect
reconstruction but a nonzero orthogonality defect, so reconstruction alone
cannot certify a QR factorization.  Neither diagnostic, even when small, proves
the exact rank of floating-point data.
\end{solution}
