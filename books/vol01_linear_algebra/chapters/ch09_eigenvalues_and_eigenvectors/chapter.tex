...
% VOL01-DEPENDENCY-PREVIEW-I09-DIAGONALIZATION
\begin{remark}[Preview of Chapter 11]
This chapter uses \emph{diagonalizable} descriptively when comparing eigenspace
dimensions. Chapter~11 gives the formal criteria in terms of eigenbases and the
minimal polynomial.
\end{remark}
...
\begin{problem}[Defective shear]\label{prob:i09-problem-01}
...
\begin{problem}[Rotation and complexification]\label{prob:i09-problem-02}
...
\begin{problem}[Parameter collision]\label{prob:i09-problem-03}
...
\begin{problem}[Eigenvectors of powers]\label{prob:i09-problem-04}
...
\begin{problem}[Zero eigenvalue and singularity]\label{prob:i09-problem-05}
...
\begin{problem}[Distinct eigenvectors]\label{prob:i09-problem-06}
...
\begin{problem}[Triangular spectrum]\label{prob:i09-problem-07}
...
\begin{problem}[Eigenspace as kernel]\label{prob:i09-problem-08}
...
\begin{problem}[Eigenvalue of an inverse]\label{prob:i09-problem-09}
...
\begin{problem}[Trace and eigenvalues]\label{prob:i09-problem-10}
...
\begin{problem}[Determinant and eigenvalues]\label{prob:i09-problem-11}
...
\begin{problem}[Spectral synthesis]\label{prob:i09-problem-12}
...
\end{exercise}
\begin{hint}
If the zero vector were allowed, it would satisfy \(T0=\lambda0\) for every scalar \(\lambda\); explain why that carries no spectral information.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Use \(E_\lambda=\ker(T-\lambda I)\) and the fact that kernels of linear maps are subspaces.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Compare the roots of the same characteristic polynomial over \(\mathbb R\) and over \(\mathbb C\).
\end{hint}
\begin{solution}
