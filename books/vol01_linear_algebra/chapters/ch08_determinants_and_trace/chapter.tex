...
\begin{proof}
For fixed \(A\), define \(F_A(B)=\det(AB)\). If the columns of \(B\) are
\(b_1,\ldots,b_n\), then the columns of \(AB\) are \(Ab_1,\ldots,Ab_n\).
Therefore \(F_A\) is alternating and multilinear in the columns of \(B\).
Moreover
\[
F_A(I)=\det(A).
\]
By the uniqueness characterization of the determinant, the unique alternating
multilinear function of the columns of \(B\) taking the value \(\det(A)\) at
\(I\) is \(\det(A)\det(B)\). Hence
\[
\det(AB)=\det(A)\det(B).
\]
\end{proof}
...
\begin{problem}[Determinant by elimination]\label{prob:i08-problem-01}
...
\begin{problem}[Effect of a row replacement]\label{prob:i08-problem-02}
...
\begin{problem}[Area scaling]\label{prob:i08-problem-03}
...
\begin{problem}[Invertibility test]\label{prob:i08-problem-04}
...
\begin{problem}[Trace cyclicity]\label{prob:i08-problem-05}
...
\begin{problem}[Similarity invariants]\label{prob:i08-problem-06}
...
\begin{problem}[Determinant of a reflection]\label{prob:i08-problem-07}
...
\begin{problem}[Determinant of a shear]\label{prob:i08-problem-08}
...
\begin{problem}[Trace of rank one]\label{prob:i08-problem-09}
...
\begin{problem}[Block triangular determinant]\label{prob:i08-problem-10}
...
\begin{problem}[Orientation test]\label{prob:i08-problem-11}
...
\begin{problem}[Determinant synthesis]\label{prob:i08-problem-12}
...
\end{exercise}
\begin{hint}
For a diagonal matrix, only the identity permutation contributes to the determinant.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Use cyclicity \(\operatorname{tr}(XY)=\operatorname{tr}(YX)\) with a factorization that places \(P^{-1}P\) together.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Choose a basis consisting of one normal direction and a basis of the reflecting hyperplane, then read the diagonal matrix.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Write the diagonal entries of \(uv^T\) explicitly and add them.
\end{hint}
\begin{solution}
...
\end{exercise}
\begin{hint}
Two matrices of the same endomorphism in different bases are similar; use multiplicativity of determinant and cyclicity of trace.
\end{hint}
\begin{solution}
