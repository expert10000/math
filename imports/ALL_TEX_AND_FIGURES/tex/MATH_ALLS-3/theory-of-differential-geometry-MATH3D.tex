...
\begin{document}
	...
	\section{Example 1: Zariski topology on $\mathbb A^1$ and the product topology on $\mathbb A^1\times \mathbb A^1$}
	
	Assume $k$ is algebraically closed.
	
	\subsection{Affine line $\mathbb A^1$}
	
	Let
	\[
	\mathbb A^1=\mathrm{Spec}(k[t]).
	\]
	For an ideal $I\subset k[t]$ define the algebraic set
	\[
	Z(I)=\{a\in k \mid f(a)=0 \text{ for all } f\in I\}.
	\]
	...
	\]
	Over an algebraically closed field, a nonzero polynomial $f$ has finitely many roots, hence:
	\[
	\text{closed sets in } \mathbb A^1 \text{ are } \emptyset,\ \mathbb A^1,\ \text{and finite subsets of }k.
	\]
	...
	Then:
	\[
	D(f)\text{ is open, and these }D(f)\text{ form a basis.}
	\]
	Example:
	\[
	f(t)=t(t-1),
	...
	Give each factor $\mathbb A^1$ the Zariski topology (cofinite topology).
	Let
	\[
	X=\mathbb A^1\times \mathbb A^1
	\]
	with the product topology.
	
	\subsubsection{Basic opens and basic closed sets}
	
	A basis of opens is
	\[
	U\times V
	\]
	where $U,V\subset \mathbb A^1$ are Zariski open.
	Since $U,V$ are complements of finite sets, any basic open has the form
	\[
	(k\setminus F)\times (k\setminus G)
	\]
	with $F,G$ finite.
	
	A typical closed set in the product topology is a finite union of sets of the form
	\[
	F\times \mathbb A^1,
	\qquad
	\mathbb A^1\times G,
	...
	\subsubsection{Comparison with the Zariski topology on $\mathbb A^2$}
	
	In $\mathbb A^2$ (Zariski topology coming from $k[x,y]$), the diagonal
	\[
	\Delta=Z(y-x)
	\]
	is Zariski closed.
	But $\Delta$ is not closed in the product topology, because product-closed sets are built from finitely many vertical and horizontal fibers and finite sets.
	
	Hence:
	\[
	\text{product topology on }\mathbb A^1\times\mathbb A^1\text{ is strictly coarser than the Zariski topology on }\mathbb A^2.
	\]
